...
CServerDlg::CServerDlg()
{
	...
	m_iAmount = 0;
	m_bIsNight = false;
	m_byBattleOpen = BATTLEZONE_CLOSE;
	m_sKillKarusNpc = 0;
	m_sKillElmoNpc = 0;
}
...
void CServerDlg::CheckAliveTest()
{
	...
	uint32 count = 0, sessCount = (uint32) sessMap.size();
	foreach (itr, sessMap)
	{
...
void CServerDlg::DeleteAllUserList(CGameSocket *pSock)
{
	...
	printf("*** DeleteAllUserList - Start *** \n");
	foreach_stlmap (itr, g_arZone)
	{
		MAP * pMap = itr->second;
		if (pMap == nullptr)	
...
CNpc * CServerDlg::SpawnEventNpc(uint16 sSid, bool bIsMonster, uint8 byZone, float fX, float fY, float fZ, uint8 byDirection, uint16 sDuration, uint8 nation, int16 socketID, uint16 nEventRoom)
{
	if (m_TotalNPC >= INVALID_BAND)
		return false;
