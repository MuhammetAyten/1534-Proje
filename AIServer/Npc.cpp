	...
	m_proto(nullptr), m_pPath(nullptr)
{
	...
	m_NpcOldActType = ATROCITY_ATTACK_TYPE; //
	m_NpcGroupType = 0; //
	m_NpcEndActType = 1; //
	m_byMoveType = 1;
	m_byInitMoveType  = 1;
	...
	m_bTracing = false; // monster speed
...
void CNpc::Load(uint16 sNpcID, CNpcTable * proto, bool bMonster, uint8 nation)
{
	...
	if (GetType() == NPC_DESTROYED_ARTIFACT && GetProtoID() == 541)
...
void CNpc::SendMoveResult(float fX, float fY, float fZ, float fSpeed)
{
...
time_t CNpc::NpcStanding()
{
	...
	if (GetType() == NPC_SPECIAL_GATE && GetMap()->isWarZone() && g_pMain->m_byBattleOpen == BATTLEZONE_OPEN ||
		GetType() == NPC_SPECIAL_GATE && GetMap()->isWarZone() && g_pMain->m_byBattleOpen == SNOW_BATTLE)
	{
		...
		else if (!m_byGateOpen && UnixGateOpen == 40)
		{
			m_byGateOpen = !m_byGateOpen;
			Packet result(AG_NPC_GATE_OPEN);
			result << GetID() << GetProtoID() << m_byGateOpen;
	...
	else if (GetType() == NPC_SPECIAL_GATE && GetZoneID() == ZONE_DELOS && g_pMain->m_byBattleOpen == SIEGE_BATTLE)
	{
		if(m_byGateOpen && UnixGateClose == 30)
		{
			m_byGateOpen = !m_byGateOpen;
	...
	else if (GetType() == NPC_OBJECT_WOOD && GetMap()->isWarZone() && g_pMain->m_byBattleOpen == BATTLEZONE_OPEN)
	{
		if (m_byGateOpen && UnixGateClose++ >= 60)
		{
			m_byGateOpen = !m_byGateOpen;
			m_NpcState = SetLive() ? NPC_STANDING : NPC_LIVE;
			Packet result(AG_NPC_GATE_OPEN);
	...
	{
		if (m_byGateOpen && UnixGateClose++ >= m_sRegenTime / 1000)
		{
			m_byGateOpen = !m_byGateOpen;
			m_NpcState = SetLive() ? NPC_STANDING : NPC_LIVE;
			Packet result(AG_NPC_GATE_OPEN);
...
bool CNpc::RandomMove()
{
	...
	{
		random_x = myrand((int)-m_fSpeed_1, (int)m_fSpeed_1);
		random_z = myrand((int)-m_fSpeed_1, (int)m_fSpeed_1);
		...
		case 0:
			{
				...
				case 5:
					{
						if ((GetType() == NPC_ROLLINGSTONE && GetZoneID() == ZONE_KROWAZ_DOMINION) 
							|| (GetType() == NPC_OBJECT_WOOD && m_byGateOpen && GetZoneID() == ZONE_BATTLE6))
						{
							fDestX = (float)m_PathList.pPattenPos[m_sPathCount].x + m_fBattlePos_x;
							fDestZ = (float)m_PathList.pPattenPos[m_sPathCount].z + m_fBattlePos_z;
	...
	if (m_byMoveType == 6/*Orjinal 4*/ && (GetProtoID() == 19070 || GetProtoID() == 19071 || GetProtoID() == 19072))
	{
		IsNoPathFind(m_fSecForMetor);
		return true;
	}
...
bool CNpc::RandomBackMove()
{
	...
	if (nID < NPC_BAND)
	{
		...
		{
			GetVectorPosition(vStart, vEnd, m_fSecForMetor, &vEnd22);
			fDestX = vEnd22.x;
			fDestZ = vEnd22.z;
		}
	}
...
bool CNpc::FindEnemy()
{
	...
	bool bIsNeutralZone = (GetZoneID() >= ZONE_MORADON && GetZoneID() <= ZONE_MORADON5 || GetZoneID() == ZONE_ARENA); 
		...
		foreach_stlmap (itr, pRegion->m_RegionUserArray)
		{
			...
			if (m_tNpcAttType == ATROCITY_ATTACK_TYPE
				// Passive spawns will only attack if they've been attacked first, or they've got backup! (Cowards!)
					|| (m_tNpcAttType == TENDER_ATTACK_TYPE && (IsDamagedUserList(pUser) || (m_NpcGroupType && m_Target.id == target_uid)))) //
			{
...
bool CNpc::StepMove()
{
	...
	if(m_fSecForRealMoveMetor > m_fSecForMetor)
	{
...
CloseTargetResult CNpc::IsCloseTarget(int nRange, AttackType attackType)
{
	...
	if ((int)fDis > nRange+20 && attackType == 1 && m_bStopFollowingTarget == true)
	{
		InitTarget();
		NpcMoveEnd();
...
void CNpc::ChangeTarget(int nAttackType, CUser *pUser)
{
	...
	{
		m_Target.failCount = 0; //
		if (m_NpcGroupType || GetType() == NPC_BOSS) //
	...
	if (m_NpcState != NPC_FAINTING)
	{
		...
		switch (attributeType)
		{
			...
			case 6:
				...
				break;
			default:
				sDamage = (int)(10 + (40 - 40 * ((double)nDamage / 80)));
				break;
		}
...
void CNpc::SendExpToUserList()
{
	...
	if (g_pMain->m_byBattleOpen == BATTLEZONE_OPEN
	...
	foreach_stlmap (itr, pRegion->m_RegionNpcArray)
	{
		...
		if (pNpc != nullptr && pNpc->m_NpcState != NPC_DEAD && pNpc->GetID() != GetID())
		{
			...
			if (fDis <= fSearchRange)	
			{
				...
				{
					if (pNpc->m_NpcGroupType && GetID() != pNpc->GetID() && pNpc->m_proto->m_byFamilyType == m_proto->m_byFamilyType)//	
					{
						if (pNpc->hasTarget() && pNpc->m_NpcState == NPC_FIGHTING) 
							continue;
...
bool CNpc::IsPathFindCheck(float fDistance)
{
	...
	do
	{
		...
		if (pMap->IsMovable(nX, nZ) || count >= MAX_PATH_LINE)
		{
			nError = -1;
			break;
		}
...
{  
	...
	{
	   m_curx = (cPost.x / bChamber / 3) + oPos.x;
       m_curz = (cPost.z / bChamber / 3) + oPos.z;
	}
	else if (m_bSpeedAmount <=100)
	{
	   m_curx = (cPost.x / 2) + oPos.x;
	   m_curz = (cPost.z / 2) + oPos.z;
	}
		...
		for (int j = 0; j < 5; j++)
		{
			...
			if (j < 2)
			{
				...
				else 
				{
					...
					if (pGroup == nullptr || pGroup->iItems.size() != 100)
						continue;

					iMakeItemCode = pGroup->iItems[myrand(1, 100) - 1];
				}
...
time_t CNpc::NpcSleeping()
{
	...
	m_NpcState = NPC_SLEEPING;
	result << GetID() << uint8(1) << m_NpcState;
	g_pMain->Send(&result);
	return m_sStandTime;
}
...
void CNpc::ChangeAbility(int iChangeType)
{
	...
	if (iChangeType == BATTLEZONE_OPEN || iChangeType == NATION_BATTLE || iChangeType == SIEGE_BATTLE)
	{
		nHP = (int)(pNpcTable->m_iMaxHP);
