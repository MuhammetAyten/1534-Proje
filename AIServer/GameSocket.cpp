...
void CGameSocket::RecvServerConnect(Packet & pkt)
{
	...
	printf("**** GameSocket Connect Success server = %s:%d ****\n", GetRemoteIP().c_str(), GetRemotePort());
...
void CGameSocket::RecvUserMove(Packet & pkt)
{
	...
	uint16 uid, speed;
	float fX, fZ, fY;
	pkt >> uid >> fX >> fZ >> fY >> speed >> type;
	SetUid(fX, fZ, uid, type, speed);
}

bool CGameSocket::SetUid(float x, float z, int id, uint8 type, int speed)
{
	...
	if (pUser != nullptr)
	{
		...
		if (type == Unit::TransformationNone)
			pUser->m_transformationType = Unit::TransformationNone;
		if(type == Unit::TransformationMonster)
			pUser->m_transformationType = Unit::TransformationMonster;
		if(type == Unit::TransformationNPC)
			pUser->m_transformationType = Unit::TransformationNPC;
		if(type == Unit::TransformationSiege)
			pUser->m_transformationType = Unit::TransformationSiege;
	}
...
void CGameSocket::RecvZoneChange(Packet & pkt)
{
	...
	uint16 m_bEventRoom = pkt.read<uint16>();
	...
	pUser->m_bEventRoom = m_bEventRoom;

	TRACE("**** RecvZoneChange -- user(%s, %d), cur_zone = %d, EventRoom %d \n", pUser->GetName().c_str(), pUser->GetID(), byZoneNumber, m_bEventRoom);
}
...
void CGameSocket::RecvBattleEvent(Packet & pkt)
{
	...
	if (bEvent == BATTLEZONE_OPEN || bEvent == BATTLEZONE_CLOSE || bEvent == SIEGE_BATTLE || bEvent == SNOW_BATTLE)
	{
		g_pMain->m_sKillKarusNpc = 0;
		g_pMain->m_sKillElmoNpc = 0;
		g_pMain->m_byBattleOpen = bEvent;

		if (bEvent == BATTLEZONE_CLOSE || bEvent == SNOW_BATTLEZONE_CLOSE)
	...
	foreach_stlmap (itr, g_pMain->m_arNpc)
	{
		...
		{
			if (bEvent == BATTLEZONE_OPEN || bEvent == BATTLEZONE_CLOSE || bEvent == SIEGE_BATTLE || bEvent == SNOW_BATTLE || bEvent == SNOW_BATTLEZONE_CLOSE)
...
void CGameSocket::RecvNpcSpawnRequest(Packet & pkt)
{
	...
	uint16 nEventRoom;
	uint8 byZone;
	uint8 nation,byDirection;
	bool bIsMonster;
	float fX, fY, fZ;

	pkt >> sSid >> bIsMonster >> byZone >> sX >> sY >> sZ >> byDirection >> sCount >> sRadius >> sDuration >> nation >> socketID >> nEventRoom;
	...
	for (uint16 i = 0; i < sCount; i++)
	{
		g_pMain->SpawnEventNpc(sSid, bIsMonster, 
			...
			fY, 
			(float)(fZ + myrand(minRange, sRadius)), byDirection, sDuration, nation, socketID, nEventRoom);
	}
}
...
void CGameSocket::RecvNpcUpdateInfo(Packet & pkt)
{
	uint16 sNid;
	uint8 byCode, m_NpcState;
	float GetX, GetY, GetZ, fSpeed;

	pkt >> sNid >> byCode >> m_NpcState >> GetX >> GetY >> GetZ >> fSpeed;
	...
	{
	case 1:
		pNpc->m_NpcState = m_NpcState;
		break;
	case 2:
		pNpc->SendMoveResult(GetX, GetY, GetZ, fSpeed);
		break;
	default:
		break;
	}
