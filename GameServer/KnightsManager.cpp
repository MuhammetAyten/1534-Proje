...
void CKnightsManager::JoinKnights(CUser *pUser, Packet & pkt)
{
	...
	do
	{
		...
		result	<< uint8(KNIGHTS_JOIN_REQ) << uint8(1)
			<< pUser->GetSocketID() << sClanID << pKnights->GetName() << pKnights->m_sMarkVersion;
		pTUser->Send(&result);
		return;
	} while (0);
...
void CKnightsManager::WithdrawKnights(CUser *pUser, Packet & pkt)
{
	...
	CKnights * pKinghts = g_pMain->GetClanPtr(pUser->GetClanID());
	Packet result(WIZ_KNIGHTS_PROCESS);
	uint8 bResult = 0;
	uint16 m_sCapeID;
	do
	{
		if (!pUser->isInClan() || pKinghts == nullptr)
			bResult = 10;
		...
		if (pKinghts->isInAlliance())
		{
			if (!g_DBAgent.LoadCapeId(pKinghts->GetID(), m_sCapeID))
				return;

			result << (uint8) KNIGHTS_ALLY_REMOVE << uint8(ClanTypeTraining) << pKinghts->GetAllianceID() << pUser->GetClanID() << m_sCapeID;
			g_pMain->AddDatabaseRequest(result, pUser);
			return;
		}
...
void CKnightsManager::UpdateKnightsGrade(uint16 sClanID, uint8 byFlag)
{
	...
	{
		CUser * pUser = g_pMain->GetUserPtr(pClan->m_strChief, TYPE_CHARACTER);

		if(pUser == nullptr)
			return;

		if (!g_DBAgent.LoadCapeId(pClan->GetID(), m_sCapeID))
			return;

		result << (uint8) KNIGHTS_ALLY_REMOVE << uint8(ClanTypeTraining) << pClan->GetAllianceID() << pClan->GetID() << m_sCapeID;
		g_pMain->AddDatabaseRequest(result, pUser);
		return;
	}
	...
	result << (uint8) KNIGHTS_UPDATE_GRADE << sClanID << byFlag << pClan->m_sCape;
...
void CKnightsManager::KnightsAllianceCreate(CUser* pUser, Packet & pkt)
{
	...
	if (pKnights == nullptr)
		bResult = 0;
	else if (pKnights->m_byFlag < 1)
		bResult = 0;
	else if (pKnights->m_strChief != pUser->GetName())
		bResult = 0;
	else if (g_pMain->m_nServerGroup == 2)
		bResult = 0;
	else if (!pUser->GetMap()->canUpdateClan())
		bResult = 0;
	else if (pUser->GetTargetID() > NPC_BAND)
		bResult = 0;
	else
		bResult = 1;
	...
	if (bResult == 1)
	{
		result << uint8(KNIGHTS_ALLY_CREATE) << bResult << pUser->GetClanID();
		g_pMain->AddDatabaseRequest(result, pUser);
		return;
	}
	result << (uint8) KNIGHTS_ALLY_CREATE << bResult;
	pUser->Send(&result);
}
...
void CKnightsManager::KnightsAllianceInsert(CUser* pUser, Packet & pkt)
{
	...
	do
	{
		if (!pUser->GetMap()->canUpdateClan())
			...
			bResult = 2;
		else if (!pUser->isInClan() || !pUser->isClanLeader())
			bResult = 6;
		...
		CUser *pTUser = g_pMain->GetUserPtr(pUser->GetTargetID());
		if (pTUser == nullptr)
			bResult = 2;
		else if (pTUser->isDead())
			bResult = 3;
			...
			bResult = 4;
		else if (!pTUser->isInClan() || !pTUser->isClanLeader())
			bResult = 5;

		if (bResult != 0)
			break;

		CKnights *pClan = g_pMain->GetClanPtr(pTUser->GetClanID());
		
		if (pClan == nullptr)
			bResult = 2;
		else if (pClan->isInAlliance())
			bResult = 5;

		if (bResult != 0)
			break;

		pUser->m_AlianceID = pTUser->GetSocketID();
		pTUser->m_AlianceID = pUser->GetSocketID();
		result << uint8(KNIGHTS_ALLY_REQ);
		result << uint8(ClanTypeTraining) << pKnights->GetName();
		pTUser->Send(&result);
		return;
	} while (0);
...
void CKnightsManager::KnightsAllianceRequest(CUser* pUser, Packet & pkt)
{
	...
	CUser * pTUser = g_pMain->GetUserPtr(pUser->m_AlianceID);
	...
	CKnights * pKnights = g_pMain->GetClanPtr(pTUser->GetClanID());
	if(pKnights == nullptr)
	{
		bResult = 2;
		goto fail_return;
	...
	if(pAlliance == nullptr)
	{
		if (pKnights->m_byFlag < 1)
			return;
		else if (pKnights->m_strChief != pTUser->GetName())
			return;
		else if (g_pMain->m_nServerGroup == 2)
			return;
		else if (!pTUser->GetMap()->canUpdateClan())
			return;
		else if (pTUser->GetTargetID() > NPC_BAND)
			return;

		result.SetOpcode(WIZ_KNIGHTS_PROCESS);
		result << uint8(KNIGHTS_ALLY_CREATE) << uint8(ClanTypeTraining) << pTUser->GetClanID();
		g_pMain->AddDatabaseRequest(result, pTUser);
		return;
	}
	if (pTUser->isDead())
	{
	...
	if (pKnights->m_byFlag > 1 && 
		!pClan->isInAlliance() &&
		pKnights->m_strChief == pTUser->GetName() &&
		pAlliance != nullptr &&
		pAlliance->sMainAllianceKnights == pKnights->GetAllianceID() &&
		pAlliance->sMercenaryClan_1 != pClan->m_sIndex &&
		pAlliance->sMercenaryClan_2 != pClan->m_sIndex &&
		pAlliance->sSubAllianceKnights != pClan->m_sIndex)
	{
		result.SetOpcode(WIZ_KNIGHTS_PROCESS);
		result << uint8(KNIGHTS_ALLY_INSERT) << uint8(ClanTypeTraining) << pUser->GetClanID() << pTUser->GetClanID() << pKnights->m_sCape;
		g_pMain->AddDatabaseRequest(result, pUser);
		std::string noticeText;
		g_pMain->GetServerResource(IDS_INSERT_KNIGHTS_ALLIANCE, &noticeText, pClan->GetName().c_str());
...
{	
	...
	if (!g_DBAgent.LoadCapeId(pClan->GetID(), m_sCape))
		return;

	CKnights * pKnights = g_pMain->GetClanPtr(pClan->GetAllianceID());

	if (pKnights == nullptr)
		return;

	_KNIGHTS_ALLIANCE * pAlliance = g_pMain->GetAlliancePtr(pClan->GetAllianceID());
	...
	if (pAlliance != nullptr)
	{
		...
		if( pClan->m_strChief == pUser->GetName() ||
			pKnights->m_strChief == pUser->GetName() &&
			!pKnights->isInAlliance() &&
			pAlliance != nullptr &&
			pAlliance->sMainAllianceKnights == pClan->GetAllianceID() ||
			pAlliance->sMercenaryClan_1 == pClan->m_sIndex ||
			pAlliance->sMercenaryClan_2 == pClan->m_sIndex ||
			pAlliance->sSubAllianceKnights == pClan->m_sIndex)
		{
			result << uint8(ClanTypeTraining) << pClan->GetAllianceID() << pUser->GetClanID() << m_sCape;
			g_pMain->AddDatabaseRequest(result, pUser);
			std::string noticeText;
...
{	
	...
	uint16 nClanID, nCapeID;

	pkt >> nClanID;

	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KNIGHTS_ALLY_PUNISH)); // Knights Alliance Banned

	CKnights * pKnights = g_pMain->GetClanPtr(pUser->GetClanID());

	if (pKnights == nullptr)
		return;
	else if (!pKnights->isAllianceLeader())
		return;

	CKnights * pClan = g_pMain->GetClanPtr(nClanID);
	...
	if (!g_DBAgent.LoadCapeId(pClan->GetID(), nCapeID))
		return;
	...
	{
		result << uint8(1) << pKnights->GetID() << pClan->GetID() << nCapeID;
		g_pMain->AddDatabaseRequest(result, pUser);
		std::string noticeText;
		g_pMain->GetServerResource(IDS_PUNISH_KNIGHTS_ALLIANCE, &noticeText, pClan->GetName().c_str());
		pKnights->SendChatAlliance("%s", noticeText.c_str());
		if(pKnights1 != nullptr)
			pKnights1->SendChatAlliance("%s", noticeText.c_str());
		if(pKnights2 != nullptr)
...
void CKnightsManager::ListTop10Clans(CUser *pUser)
{
	...
	// Top 10 Clan Listesi 5 Karus 5 Human Fixed
	...
	case 2:
		{
			...
			std::string s_AllianceNotice;

			pkt >> s_AllianceNotice;

			if(s_AllianceNotice.empty() 
				|| s_AllianceNotice.length() > 200)
				return;

			CKnights * pClanAlliance = g_pMain->GetClanPtr(pKnights->GetAllianceID());

			if (pClanAlliance == nullptr ||
				!pClanAlliance->isInAlliance())
				return;

			// Update the stored Alliance notice
			pClanAlliance->m_strAllianceNotice = s_AllianceNotice;

			// Construct the update notice packet to inform players the clan notice has changed
			std::string s_UpdateAllianceNotice = string_format("%s updated the alliance notice.", pKnights->m_strChief.c_str()); 
			ChatPacket::Construct(&result, ALLIANCE_CHAT, &s_UpdateAllianceNotice);
			...
			result << uint8(ALLIANCE_NOTICE) << pClanAlliance->GetID() << s_AllianceNotice;
			g_pMain->AddDatabaseRequest(result);
		}
		break;
	case 3:
		{
			...
			std::string strUserMemo;

			pkt >> strUserMemo;

			if(strUserMemo.empty() 
				|| strUserMemo.length() > MAX_ID_SIZE)
				return;

			// Update the stored Users Memo
			pUser->m_strMemo = strUserMemo;
			p->strUserMemo = strUserMemo;

			// Construct the update memo packet to inform players the clan users memo has changed
			result.Initialize(WIZ_KNIGHTS_PROCESS);
			result.DByte();
			result << uint8(KNIGHTS_MEMO) << uint8(0x03) << uint8(0x01) << strUserMemo;
			pUser->Send(&result);
		}
		break;
	default:
