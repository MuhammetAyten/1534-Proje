...
SkillUseResult MagicInstance::UserCanCast()
{
	...
	if (pSkillCaster->isPlayer())
	{
		...
		if (pSkillCaster->isInTempleEventZone() && 
			!g_pMain->pTempleEvent.isAttackable[pSkillCaster->GetEventRoom()] && 
		...
		if (pSkillTarget != nullptr)
		{
			if (!pSkillTarget->isPlayer())
			{
				if (TO_NPC(pSkillTarget)->GetType() == NPC_TREE)
					...
					TO_NPC(pSkillTarget)->GetType() == NPC_BORDER_MONUMENT))
					return SkillUseFail;

				else if (TO_NPC(pSkillTarget)->GetType() == TO_USER(pSkillCaster)->GetNation() &&
					TO_NPC(pSkillTarget)->GetType() == NPC_BORDER_MONUMENT)
					return SkillUseFail;
			}
		}
...
SkillUseResult MagicInstance::CheckSkillPrerequisites()
{
	...
	if (pSkillCaster->isPlayer())
	{
		...
		if (pCaster)
		{
			...
			{
				bool isManaSkills = (nSkillID >= 490016 && nSkillID <= 490020
					|| nSkillID == 490072 || nSkillID >= 490078 && nSkillID <= 490082
					|| nSkillID == 490701 || nSkillID == 492026 || nSkillID == 500007 
					|| nSkillID == 500008 || nSkillID == 500009
					|| nSkillID == 500135 || nSkillID == 500136 || nSkillID == 500137 
					|| nSkillID == 500142 || nSkillID == 500143
					|| nSkillID == 500144 || nSkillID == 500146 || nSkillID == 490140 
					|| nSkillID == 500219);
			...
			if (pSkill->bType[0] == 4 && pSkill->bType[1] == 0)
			{
				...
				else if(pType4->bBuffType == BUFF_TYPE_FISHING)
				{
					...
					if (TO_USER(pSkillCaster)->m_FlashWarBonus == 100)
						return SkillUseFail;
				}
			}
			...
			if (bOpcode != MAGIC_TYPE4_EXTEND && pCaster->m_CoolDownList.find(nSkillID) != pCaster->m_CoolDownList.end())
			{
				...
				if (DiffrentMilliseconds > 0 && DiffrentMilliseconds < int64(pSkill->sReCastTime * 100) && pSkill->bType[0] != 9 && !bIsRecastingSavedMagic)
					return SkillUseFail;
				else
	...
	if (pSkillTarget != nullptr)
	{
		...
		{
			if (pSkillTarget->isPlayer() && (pSkillTarget->hasBuff(BUFF_TYPE_FREEZE))) // Effect Sorunu ve Blink Sorunu Var...
				return SkillUseFail;
		}
	}
...
bool MagicInstance::IsAvailable()
{
	...
	case MORAL_SIEGE_WEAPON:
		if (pSkillCaster->isNPC() 
			|| (pSkillCaster->isPlayer() && !TO_USER(pSkillCaster)->isSiegeTransformation()))
			goto fail_return;
		break;
	}
	...
	for (int i = 0; i < 2; i++) 
	{
		...
		case 6:
			if (!CheckType6Prerequisites())
				return false;
			else if (TO_USER(pSkillCaster)->CheckExistItem(381001000))
...
{	
	...
	if (pType->bDirectType == 18)
	{
		...
		if (!pSkillCaster->isPlayer() || !TO_USER(pSkillCaster)->hasFullAngerGauge())
			return false;
	...
	foreach (itr, casted_member)
	{
		...
		{
			// Success rate...
			if (pSkill->bSuccessRate < 100 
				&& pSkill->bSuccessRate <= myrand(0, (100 - pSkill->bSuccessRate) * 10))
		...
		if (pType->bDuration == 0) 
		{
			...
			case 1:
				...
				if (damage > 0 && pSkillCaster->hasBuff(BUFF_TYPE_DAMAGE_DOUBLE) && CheckPercent(500))
					damage *= 2;
		...
		{
			sData[1] = 0;
			BuildAndSendSkillPacket(pSkillCaster, true, sCasterID, pTarget->GetID(), MAGIC_EFFECTING, nSkillID, sData);
		}
		else
		{
			if (bSendLightningStunSkill 
				...
				|| LightStunSkills()))
			{
					...
					|| ColdSkillsNot()))
				{
					...
					Type4BuffMap::iterator buffItr = pTarget->m_buffMap.find(pType2->bBuffType);
					...
					if (bSkillTypeAlreadyOnTarget && pType2->isDebuff())
					{
						CMagicProcess::RemoveType4Buff(pType2->bBuffType, pTarget, false);
					...
					if (!bSkillTypeAlreadyOnTarget)
					{
						...
						pBuffInfo.m_bBuffType = pType2->bBuffType;
						pBuffInfo.m_bIsBuff = pType2->bIsBuff;

						pBuffInfo.m_bDurationExtended = false;
						pBuffInfo.m_tEndTime = UNIXTIME + pType2->sDuration;

						// Add the buff into the buff map.
						pTarget->AddType4Buff(pType2->bBuffType, pBuffInfo);
					}
					...
					int16 sDataCopy[] = 
					{
						...
						sData[4], nTargetSpeedAmount, sData[6]
					};
...
bool MagicInstance::ExecuteType4()
{
	...
	foreach (itr, casted_member)
	{
		...
		if (pTarget->isPlayer())
		{
			if (TO_USER(pTarget)->isInPKZone() 
				&& pSkillCaster->GetNation() != pTarget->GetNation())
				TO_USER(pTarget)->RemoveStealth();
			else if (TO_USER(pTarget)->BorderTempleEventZone() 
				&& pSkillCaster->GetNation() != pTarget->GetNation())
				TO_USER(pTarget)->RemoveStealth();
			else if (TO_USER(pTarget)->JuraidTempleEventZone() 
				&& pSkillCaster->GetNation() != pTarget->GetNation())
				TO_USER(pTarget)->RemoveStealth();
			else if (TO_USER(pTarget)->isInClan() 
				&& TO_USER(pTarget)->GetZoneID() == ZONE_DELOS 
				&& g_pMain->m_byBattleOpen == SIEGE_BATTLE)
			{
				CKnights * pTKnights = g_pMain->GetClanPtr(TO_USER(pTarget)->GetClanID());

				if (TO_USER(pSkillCaster)->GetClanID() != TO_USER(pTarget)->GetClanID() 
					|| TO_USER(pSkillCaster)->GetClanID() != pTKnights->GetAllianceID())
					TO_USER(pTarget)->RemoveStealth();
			}
			else if (TO_USER(pTarget)->isInArena())
			{
				if (!TO_USER(pTarget)->isInParty())
					TO_USER(pTarget)->RemoveStealth();
				else if (TO_USER(pTarget)->isInParty() 
					&& TO_USER(pSkillCaster)->GetPartyID() != TO_USER(pTarget)->GetPartyID())
					TO_USER(pTarget)->RemoveStealth();
			}
		}
		...
		if (pTarget->isPlayer())
		{
				...
				|| pType->bBuffType == BUFF_TYPE_STUN))
			{
				...
				if (bSetSpeed)
				{
					...
					auto itr = pTarget->m_buffMap.find(pType->bBuffType);

					if (itr != pTarget->m_buffMap.end() 
						&& (pTypeTarget = g_pMain->m_Magictype4Array.GetData(itr->second.m_nSkillID)))
...
fail_return:
	...
	if (sCasterID != -1 
		&& pSkill->bType[0] == 4 
		&& (pType->bBuffType == BUFF_TYPE_DECREASE_RESIST 
...
bool MagicInstance::ExecuteType6()
{
	...
	if (pType == nullptr
		// Allow NPC transformations in PVP zones
			...
			&& !(pSkillCaster->GetZoneID() >= ZONE_KARUS_ESLANT && pSkillCaster->GetZoneID() <= ZONE_KARUS_ESLANT3 
			|| pSkillCaster->GetZoneID() >= ZONE_ELMORAD_ESLANT && pSkillCaster->GetZoneID() <= ZONE_ELMORAD_ESLANT3)))
	...
	if (!bIsRecastingSavedMagic)
	{
		...
		if (pType->bUserSkillUse == Unit::TransformationMonster)
	...
	case TransformationSkillUseSiege2:
		{
			if (pType->sMaxHp == 0 && pType->sMaxHp > 0)
				pCaster->m_sMaxHPAmount = (pType->sMaxHp - 100) * (pCaster->GetMaxHealth() - pCaster->m_sMaxHPAmount) / 100;
			else
				pCaster->m_sMaxHPAmount = pType->sMaxHp;

			if (pType->sMaxMp == 0 && pType->sMaxMp > 0)
				pCaster->m_sMaxMPAmount = (pType->sMaxMp - 100) * (pCaster->GetMaxMana() - pCaster->m_sMaxMPAmount) / 100;
			else
				pCaster->m_sMaxMPAmount = pType->sMaxMp;

			if (pType->sTotalAc == 0 && pType->sTotalAc > 0)
				pCaster->m_sACPercent += (pType->sTotalAc - 100);
			else
				pCaster->m_sACAmount += pType->sTotalAc;

			pCaster->m_transformationType = Unit::TransformationSiege;

			TO_USER(pCaster)->SetUserAbility();
			TO_USER(pCaster)->Send2AI_UserUpdateInfo();

			pCaster->HpChange(pType->sMaxHp);
			pCaster->MSpChange(pType->sMaxMp);
		}
		break;
...
bool MagicInstance::ExecuteType7()
{
	...
	foreach (itr, casted_member)
	{
		...
		Guard lock(pTarget->m_buffLock);
		Type7BuffMap & buffMap = pTarget->m_type7BuffMap;
		...
		if (pType->bTargetChange == 2)
		{
			...
			buffMap.insert(std::make_pair(pType->bTargetChange, _BUFF_TYPE7_INFO(nSkillID, UNIXTIME + pType->sDuration)));
			pTarget->StateChangeServerDirect(1, NPC_SLEEPING);
		}
	}
	return true;
}
...
{	
	...
	if (sTargetID == -1)
	{
		...
		{		
			...
			if (CMagicProcess::UserRegionCheck(pSkillCaster, pTarget, pSkill, pType->sRadius, sData[0], sData[2]))
				casted_member.push_back(pTarget);
		}
	...
	foreach (itr, casted_member)
	{
		...
		case 11:	// Resurrect a dead player.
			{
				...
				TO_USER(pTUser)->HpChange(TO_USER(pTUser)->m_iMaxHp, pSkillCaster);	 // Refill HP.	
				TO_USER(pTUser)->ExpChange( pType->sExpRecover/100 ,true);    // Increase target experience.

				Packet result(AG_USER_REGENE);
				result << uint16(TO_USER(*itr)->GetSocketID()) << uint16(pTUser->GetHealth());
				g_pMain->Send_AIServer(&result);
			} break;
		...
		case 12:	// Summon a target within the zone.	
			...
			if (pSkillCaster->GetZoneID() != pTUser->GetZoneID()
				// Cannot teleport ourselves.
					|| pSkillCaster == pTUser 
					|| pTUser->isBlinking()
					|| !TO_USER(pTUser)->isInParty()
					|| TO_USER(pSkillCaster)->GetPartyID() != TO_USER(pTUser)->GetPartyID())
					goto packet_send;
		...
		case 21:	// Summon a monster within a zone with monster staff.
			{
				...
				if (pMonsterSummonList)
				{
					...
					for (std::vector<_MONSTER_SUMMON_LIST>::iterator itr = pMonsterSummonList->begin(); itr != pMonsterSummonList->end(); ++itr)
					{
						...
						{
							g_pMain->SpawnEventNpc(itr->sSid,true,pSkillCaster->GetZoneID(),pSkillCaster->GetX(),pSkillCaster->GetY(),pSkillCaster->GetZ(),0,1,(pType->sRadius/1000));
							break;
						}
		...
		case 25:
			...
			if (!TO_USER(pTUser)->isInParty() 
				|| (!pSkillCaster->isPlayer() && !TO_USER(pSkillCaster)->isInParty())
				|| (pTUser->GetZoneID() != pSkillCaster->GetZoneID()) 
				|| (TO_USER(pTUser)->GetPartyID() != TO_USER(pSkillCaster)->GetPartyID())
		...
		case 29:
			{
				...
				result.Initialize(AI_NPC_UPDATE);
				result << TO_NPC(pTUser)->GetID() << uint8(2) << TO_NPC(pTUser)->m_NpcState << warp_x << 0 << warp_z << distance;
				g_pMain->Send_AIServer(&result);
			}
			break;
		}
...
bool MagicInstance::ExecuteType9()
{
	...
	{
		buffMap.insert(std::make_pair(pType->bStateChange, _BUFF_TYPE9_INFO(nSkillID, UNIXTIME + pType->sDuration)));
		g_pMain->SpawnEventNpc(pType->sMonsterNum, true, pCaster->GetZoneID(), pCaster->GetX(), pCaster->GetY(), pCaster->GetZ(), 0, 1, 2, pType->sDuration, pCaster->GetZoneID() == ZONE_MORADON ? 3 : pCaster->GetNation(), pCaster->GetSocketID(), pCaster->GetEventRoom());
	}
	else if (pType->bStateChange >= 7 || pType->bStateChange <= 8)
	{
		...
		if(pType->bStateChange == 8)
			g_pMain->SpawnEventNpc(pType->sMonsterNum, false, pCaster->GetZoneID(), pCaster->GetX(), pCaster->GetY(), pCaster->GetZ(), 0, 1, 2, pType->sDuration, pType->bNationChange, pCaster->GetSocketID(), pCaster->GetEventRoom());
		else 
			g_pMain->SpawnEventNpc(pType->sMonsterNum, true, pCaster->GetZoneID(), pCaster->GetX(), pCaster->GetY(), pCaster->GetZ(), 0, 1, 2, pType->sDuration, pType->bNationChange, pCaster->GetSocketID(), pCaster->GetEventRoom());
...
void MagicInstance::Type6Cancel(bool bForceRemoval)
{
	...
	{
		_MAGIC_TYPE6 * pType = g_pMain->m_Magictype6Array.GetData(pUser->m_bAbnormalType);

		if (pType == nullptr)
			return;

		pUser->m_sMaxHPAmount = 0;
		pUser->m_sMaxMPAmount = 0;

		if (pType->sTotalAc == 0 && pType->sTotalAc > 0)
			pUser->m_sACPercent -= (pType->sTotalAc - 100);
		else
			pUser->m_sACAmount -= pType->sTotalAc;

		TO_USER(pUser)->SetUserAbility();
		TO_USER(pUser)->Send2AI_UserUpdateInfo();
	}
...
void MagicInstance::Type7Cancel(bool bRemoveFromMap)
{
	if (pSkillCaster == nullptr)
		return;
	...
	Type7BuffMap & buffMap = pSkillCaster->m_type7BuffMap;

	// If this skill isn't already applied, there's no reason to remove it.
	if (buffMap.find(pType->bTargetChange) == buffMap.end())
		return;
	...
	if (pType->bTargetChange == 2)
	{
		...
		pSkillCaster->StateChangeServerDirect(1, NPC_TRACING);
	}
}
