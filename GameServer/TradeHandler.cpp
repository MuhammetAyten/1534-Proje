...
void CUser::ExchangeAdd(Packet & pkt)
{
	...
	uint64 nSerialNum;
	DateTime time;
	uint32 nItemID, count, nUserSeal = 0;
	uint16 duration = 0;
...
void CUser::ExchangeDecide()
{
	...
	else
	{
		...
		foreach (itr, pUser->m_ExchangeItemList)
		{
			...
			if ((*itr)->isUserSeal())
				SendUserSealItem(result, (*itr)->nUserSeal);
			else
				result << uint32(0);
...
void CUser::ExchangeGiveItemsBack()
{
	...
	while (m_ExchangeItemList.size())
	{
		...
		if (pItem != nullptr)
		{
			...
			{
				_ITEM_DATA * pUserItem = GetItem(pItem->bSrcPos);
				if(pUserItem != nullptr && pUserItem->nNum == pItem->nItemID)
