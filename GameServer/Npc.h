...
public:
	...
	void Type4Duration();
	INLINE bool isLockableScroll(uint8 buffType) { return (buffType == BUFF_TYPE_HP_MP || buffType == BUFF_TYPE_AC || buffType == BUFF_TYPE_DAMAGE || buffType == BUFF_TYPE_FISHING); }
	...
	void SendAIServerNpcUpdate(uint8 bType, float xpos, float ypos, float zpos, float speed);
	void Update();
	...
	void BDWMonumentProcess(CUser *pUser);
