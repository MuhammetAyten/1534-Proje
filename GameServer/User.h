...
// Time (in seconds) between each reward online (1 hour).
#define PLAYER_ONLINE_REWARD      (1 * HOUR)
...
public:
	...
	uint8	m_MonsterStoneFamily;
	time_t	m_bMonsterStoneTime;
	time_t	m_MonsterSummonTime;
	bool	m_bMonsterStatus;
	bool	m_aMonsterStatus;
	bool	m_tMonsterStatus;
	...
	short	m_iMaxHp, m_iMaxMp, m_iMaxSp;
...
public:
	...
	INLINE bool  isMonsterStoneActive(){ return m_bMonsterStatus;}
	INLINE bool  isMonsterSummonActive(){ return m_aMonsterStatus;}
	INLINE bool  isMonsterStoneMonsterKill(){return m_tMonsterStatus;}

	INLINE uint16 GetGenieTime(){ return m_GenieTime; }

	INLINE uint16 GetAchieveTitle() { return m_sAchieveCoverTitle; } // Achieve Title gönderilecek.
	INLINE uint16 GetAchieveSkillTitle() { return m_sAchieveSkillTitle; } // Achieve Title gönderilecek.
	
	INLINE uint8 GetGenieStatus() { return (m_bRebStats[0]+ m_bRebStats[1]+ m_bRebStats[2]+ m_bRebStats[3]+ m_bRebStats[4])/2;} // Char Rebirth mi 
	...
	INLINE bool isInEventZone() {  return GetZoneID() == ZONE_BORDER_DEFENSE_WAR || GetZoneID() == ZONE_JURAID_MOUNTAIN || GetZoneID() == ZONE_CHAOS_DUNGEON; }
	...
	INLINE uint16 GetPremiumTime() { return m_sPremiumTime; }
	INLINE bool isLockableScroll(uint8 buffType) { return (buffType == BUFF_TYPE_HP_MP || buffType == BUFF_TYPE_AC || buffType == BUFF_TYPE_FISHING || buffType == BUFF_TYPE_DAMAGE || buffType == BUFF_TYPE_SPEED || buffType == BUFF_TYPE_STATS || buffType == BUFF_TYPE_BATTLE_CRY); }
	...
	*/
	...
	virtual int32 GetMaxStamina() { return m_iMaxSp; }
	...
	INLINE uint16 GetActiveQuestID() { return s_QuestMap.size() >= 20 ? 1 : 0; }
	...
	void DcFlash();
	void WarFlash();
	void HandleCapture(Packet & pkt);
	void HandleMovingTower(Packet & pkt);
	...
	void MonsterStoneProcess(uint8 bType = 0);
	...
	COMMAND_HANDLER(CUser::HandlePrisonCommand);
	...
	COMMAND_HANDLER(HandleNtsCommand);
	...
	void SpecialOldManExchange(Packet & pkt);
	...
	void SendClanNameChange(ClanNameChangeOpcode opcode = ClanNameChangeShowDialog);
	...
	void LogOut();
	void SendMyInfo();
	void SendUserSealItem(Packet & pkt, uint32 nUserSeal);
	...
	void MerchantUserInOutForMe(Packet & pkt);
	...
	bool RunSelectExchange(int nExchangeID, uint32 Count = 0 /* No random flag */);
	...
	void QuestV2MonsterDataDeleteAll(uint16 sQuestID);
	...
	void AddCountForKillMap(uint16 sNpcID, uint16 sQuestNum, _QUEST_MONSTER* pQuestMonster, _QUEST_DATA * pQuestKillMap);
	...
	void ReqChangeClanName(Packet & pkt);
	...
	QuestMap			m_QuestMap;
	typedef std::map<uint16, _QUEST_DATA *> QuestDataList;
	QuestDataList		s_QuestMap;
...
public:
	...
	DECLARE_LUA_GETTER(GetPremiumTime)
	...
	DECLARE_LUA_FUNCTION(CheckGiveSlot) {
		LUA_RETURN(LUA_GET_INSTANCE()->CheckGiveSlot(LUA_ARG(uint8, 2)));	// Slot Control
	}
	...
	DECLARE_LUA_FUNCTION(ResetStatPoints) {
		LUA_NO_RETURN(LUA_GET_INSTANCE()->AllPointChange(false));
	}

	DECLARE_LUA_FUNCTION(ResetSkillPoints) {
		LUA_NO_RETURN(LUA_GET_INSTANCE()->AllSkillPointChange(false));
	}
	...
	DECLARE_LUA_FUNCTION(TempleOperations) {
		LUA_NO_RETURN(LUA_GET_INSTANCE()->TempleOperations(LUA_ARG(uint8, 2)));		
	}
