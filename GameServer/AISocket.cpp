		...
		if (g_pMain->m_MonsterRespawnListInformationArray.GetSize() > 0)
		{
			foreach_stlmap_nolock (itr, g_pMain->m_MonsterRespawnListInformationArray)
			{
				if (itr->second->ZoneID != ZONE_CHAOS_DUNGEON 
					&& itr->second->ZoneID != ZONE_BORDER_DEFENSE_WAR 
					&& itr->second->ZoneID != ZONE_JURAID_MOUNTAIN )
					g_pMain->SpawnEventNpc(itr->second->sSid, itr->second->bType == 0 ? true : false ,itr->second->ZoneID,itr->second->X,itr->second->Y,itr->second->Z,itr->second->byDirection,itr->second->sCount,itr->second->bRadius);
			}
		}
...
void CAISocket::RecvNpcAttack(Packet & pkt)
{
	...
	pTarget = g_pMain->GetUnitPtr(sTargetID);
	 ...
	 if (pAttacker->m_bEventRoom != pTarget->m_bEventRoom)
		 return;
...
void CAISocket::RecvNpcGiveItem(Packet & pkt)
{
	...
	result << sNid << pBundle->nBundleID << uint8(1);
	if (!pUser->isInParty())
		pUser->Send(&result);
	else
...
void CAISocket::RecvNpcUpdateInfo(Packet & pkt)
{
	uint16 sNid;
	uint8 byCode, m_NpcState;

	pkt >> sNid >> byCode >> m_NpcState;
	...
	case 1:
		...
		pNpc->StateChangeServerDirect(byCode, m_NpcState);
		break;
	default:
		break;
	}
