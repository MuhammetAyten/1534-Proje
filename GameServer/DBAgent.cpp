...
void CDBAgent::LoadCharInfo(string & strCharID, ByteBuffer & result)
{
	...
	char strItemTime[INVENTORY_TOTAL * 4];
	ByteBuffer itemData;
...
bool CDBAgent::LoadUserData(string & strAccountID, string & strCharID, CUser *pUser)
{
	...
	char	strItem[INVENTORY_TOTAL * 8] = {0}, strSerial[INVENTORY_TOTAL * 8] = {0},
		strQuest[QUEST_ARRAY_SIZE], strItemTime[INVENTORY_TOTAL * 4], strQuestData[QUEST_LIMIT] = { 0 }, strUserSeal[INVENTORY_TOTAL * 4] = {0};;
	...
	dbCommand->FetchBinary(field++, strUserSeal, sizeof(strUserSeal));
	...
	dbCommand->FetchUInt16(field++, sQuestDataCount);
	dbCommand->FetchBinary(field++, strQuestData, sizeof(strQuestData));
	...
	pUser->s_QuestMap.clear();

	if (sQuestDataCount > QUEST_LIMIT)
		sQuestDataCount = QUEST_LIMIT;

	ByteBuffer QuestBuffer;
	QuestBuffer.append(strQuestData, sizeof(strQuestData));
	for (int i = 0, index = 0; i < sQuestDataCount; i++, index += 9)
	{
		_QUEST_DATA * pKillMap = new _QUEST_DATA();
		QuestBuffer >> pKillMap->sEventIndex;

		if (pKillMap->sEventIndex == 0)
			continue;

		for (int p = 0; p < QUEST_MOB_GROUPS; p++)
			QuestBuffer >> pKillMap->sKillCount[p];

		pUser->s_QuestMap.insert(std::make_pair(pKillMap->sEventIndex, pKillMap));
	}
	...
	ByteBuffer ItemBuffer, SerialBuffer, ItemTimeBuffer, UserSealBuffer;
	ItemBuffer.append(strItem, sizeof(strItem));
	SerialBuffer.append(strSerial, sizeof(strSerial));
	UserSealBuffer.append(strUserSeal, sizeof(strUserSeal));
	...
	{ 
		...
		if (nUserSeal == 0)
			nUserSeal = (uint32) g_pMain->GenerateItemSerial();
		...
		pItem->nUserSeal = nUserSeal;

		if (pItem->nNum == CYPHER_RING)
			pItem->bFlag = ITEM_FLAG_CHAR_SEAL;
...
{
	char strItem[WAREHOUSE_MAX * 8], strSerial[WAREHOUSE_MAX * 8], strItemTime[WAREHOUSE_MAX * 4], strUserSeal[WAREHOUSE_MAX * 4];
	...
	memset(strUserSeal, 0x00, sizeof(strUserSeal));
	memset(strItemTime, 0x00, sizeof(strItemTime));
	...
	dbCommand->FetchBinary(4, strUserSeal, sizeof(strUserSeal));
	...
	{
		uint64 nSerialNum;
		uint32 nItemID, nItemTime, nUserSeal;
		int16 sDurability, sCount;
		...
		pUser->m_sWarehouseArray[i].nUserSeal = nUserSeal;
		pUser->m_sWarehouseArray[i].nExpirationTime = nItemTime;

		if (pUser->m_sWarehouseArray[i].nNum == CYPHER_RING)
			pUser->m_sWarehouseArray[i].bFlag = ITEM_FLAG_CHAR_SEAL;
...
bool CDBAgent::UpdateUser(string & strCharID, UserUpdateType type, CUser *pUser)
{
	...
	ByteBuffer QuestKillBuffer;
	foreach (itr, pUser->s_QuestMap)
	{
		QuestKillBuffer << itr->second->sEventIndex;
		
		for (int i = 0; i < QUEST_MOB_GROUPS; i++)
			QuestKillBuffer << itr->second->sKillCount[i];

		if (++QuestDataCount >= 20)
			break;
	}

	if (QuestDataCount == 0)
		QuestKillBuffer << uint16(0) << uint64(0);

	ByteBuffer ItemBuffer, SerialBuffer, UserSealBuffer, ItemTimeBuffer;
	for (int i = 0; i < INVENTORY_TOTAL; i++)
	{
		_ITEM_DATA *pItem = &pUser->m_sItemArray[i];
		ItemBuffer << pItem->nNum << pItem->sDuration << pItem->sCount;
		SerialBuffer << pItem->nSerialNum;
		UserSealBuffer << pItem->nUserSeal;
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, (char *)UserSealBuffer.contents(), UserSealBuffer.size(), SQL_BINARY);
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, (char *)QuestKillBuffer.contents(), QuestKillBuffer.size(), SQL_BINARY);
	...
	if (!dbCommand->Execute(string_format(_T("{CALL UPDATE_USER_DATA("
		...
		"%d, %d, %d, %d, %d, %d, "			// x, z, y, dwTime, sQuestCount, QuestDatacount
		"?, ?, ?, ?, ?, ?, ?, "				// strSkill, strItem, strSerial, StrUserSeal, strItemTime, strQuest, QuestKillData
		...
		pUser->s_QuestMap.size(), pUser->m_iMannerPoint, pUser->m_iLoyaltyMonthly)))
	{
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
...
bool CDBAgent::UpdateWarehouseData(string & strAccountID, UserUpdateType type, CUser *pUser)
{
	...
	if (!dbCommand->Execute(string_format(_T("UPDATE WAREHOUSE SET nMoney=%d, dwTime=%d, WarehouseData=?, strSerial=?, strUserSeal=?, WarehouseDataTime=? WHERE strAccountID=?"), 
...
int8 CDBAgent::UpdateAlliance(uint8 byType, uint16 shAlliancIndex, uint16 shKnightsIndex, uint8  byEmptyIndex, uint8 bySiegeFlag)
{
	int8 bRet = -1;
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
...
bool CDBAgent::LoadKnightsAllMembers(uint16 sClanID, _KNIGHTS_USER* p)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return false;
	...
	if (!dbCommand->Execute(string_format(_T("SELECT Fame, [Level], Class, strMemo, DATEDIFF(HOUR,dtUpdateTime,getdate()) As dtUpdateTime FROM USERDATA WHERE Knights = %d AND strUserID = ?"), sClanID)))
		ReportSQLError(m_GameDB->GetError());
	...
	do
	{
		...
		dbCommand->FetchUInt32(5, p->m_UpdateLogin);
...
bool CDBAgent::LoadCapeId(uint16 sClanID, uint16 & m_sCape)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return false;

	if (!dbCommand->Execute(string_format(_T("SELECT sCape FROM KNIGHTS WHERE IDNum = %d" ), sClanID)))
		ReportSQLError(m_GameDB->GetError());

	if (!dbCommand->hasData())
		return false;

	dbCommand->FetchUInt16(1, m_sCape);
	return true;
}

bool CDBAgent::LoadAllianceInfo(uint16 sClanID, uint16 & sSubAllianceKnights, uint16 & sMercenaryClan_1, uint16 & sMercenaryClan_2)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return false;

	if (!dbCommand->Execute(string_format(_T("SELECT sSubAllianceKnights, sMercenaryClan_1, sMercenaryClan_2 FROM KNIGHTS_ALLIANCE WHERE sMainAllianceKnights = %d" ), sClanID)))
		ReportSQLError(m_GameDB->GetError());

	if (!dbCommand->hasData())
		return false;

	dbCommand->FetchUInt16(1, sSubAllianceKnights);
	dbCommand->FetchUInt16(2, sMercenaryClan_1);
	dbCommand->FetchUInt16(3, sMercenaryClan_2);
...
*
* @param	sClanID		 	Identifier for the clan.
* @param	strAllianceNotice	The clan notice.
*/
void CDBAgent::UpdateAllianceNotice(uint16 sClanID, std::string & strAllianceNotice)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;

	dbCommand->AddParameter(SQL_PARAM_INPUT, strAllianceNotice.c_str(), strAllianceNotice.length());
	if (!dbCommand->Execute(string_format(_T("UPDATE KNIGHTS SET strAllianceNotice = ? WHERE IDNum = %d"), sClanID)))
		ReportSQLError(m_GameDB->GetError());
}
...
ClanNameChangeOpcode CDBAgent::UpdateCharacterClanName(std::string & strClanID, std::string & strNewClanID)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return ClanNameChangeInvalidName;

	uint16 sRet = ClanNameChangePlayerRequest;
...
int8 CDBAgent::SendLetter(string & strSenderID, string & strRecipientID, string & strSubject, string & strMessage, uint8 bType, _ITEM_DATA * pItem, int32 nCoins)
{
	...
	if (!dbCommand->Execute(string_format(_T("{? = CALL MAIL_BOX_SEND(?, ?, ?, ?, %d, %d, %d, %d, " I64FMTD ", %d, %d)}"), 
		bType, nItemID, sCount, sDurability, nSerialNum, nUserSeal, nCoins)))
	{
		ReportSQLError(m_GameDB->GetError());
		return 0;
	}
...
int8 CDBAgent::GetItemFromLetter(string & strCharID, uint32 nLetterID, uint32 & nItemID, uint16 & sCount, uint16 & sDurability, uint32 & nCoins, uint64 & nSerialNum, uint32 & nUserSeal)
{
	int8 bRet = -1; // error
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
	...
	if (dbCommand->hasData())
	{
		...
		dbCommand->FetchUInt32(6, nUserSeal);
	}
...
#pragma region CDBAgent::UpdateNoahOrExpEvent(uint8 byType, uint8 byNation, uint8 byAmount, uint8 byDay, uint8 byHour, uint8 byMinute, uint16 sDuration)
...
void CDBAgent::UpdateNationIntro(uint32 iServerNo, uint8 nation, std::string & strKingName, std::string &strKingNotice)
{
	...
	if(nation == 1)
	{
		if (!dbCommand->Execute(string_format(_T("UPDATE SERVER_LIST SET strKarusKing = ?, strKarusKingNotice = ? WHERE GroupID = %d"), iServerNo)))
		{
			ReportSQLError(m_AccountDB->GetError());
			return;
		}
	...
	{
		if (!dbCommand->Execute(string_format(_T("UPDATE SERVER_LIST SET strElmoradKing = ?, strElmoradKingNotice = ? WHERE GroupID = %d"), iServerNo)))
		{
			ReportSQLError(m_AccountDB->GetError());
			return;
		}
...
bool CDBAgent::CharacterSealSave(string strCharID, uint64 bSerial, uint32 SealSerial, uint8 bRaceSatis, uint16 sClass, uint8 bLevel, uint16 pExp, uint8 & STR, uint8 & STA, uint8 & DEX, uint8 & INT, uint8 & CHA, uint32 & nGold, ByteBuffer & bstrSkill, ByteBuffer & bstrItem)
{
	...
	_USER_SEAL * pData = new _USER_SEAL;
	...
	memset(pData->Skill_Points, 0x00, sizeof(pData->Skill_Points));

	for (int i = 0; i < 9; i++)
		pData->Skill_Points[i] = bstrSkill[i];
	...
	for (int i = 0; i < INVENTORY_COSP; i++)
	{
		...
		bstrItem >> nNum >> nDuration >> nCount;

		_ITEM_TABLE *pTable = g_pMain->GetItemPtr(nNum);
...
bool CDBAgent::CharacterSealDelete(string CharID)
{
	...
	if (!dbCommand->Execute(_T("DELETE FROM USER_SEAL WHERE strUserID = ?")))
	{
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
