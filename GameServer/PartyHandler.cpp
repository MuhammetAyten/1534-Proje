...
void CUser::PartyRequest(int memberid, bool bCreate)
{
	...
	bool bIsNeutralZone = (GetZoneID() >= ZONE_MORADON && GetZoneID() <= ZONE_MORADON5 || GetZoneID() >= ZONE_FORGOTTEN_TEMPLE && GetZoneID() <= ZONE_LOST_TEMPLE);
	...
	if ((GetNation() != pUser->GetNation() && !bIsNeutralZone)
		|| GetZoneID() != pUser->GetZoneID()
		|| GetEventRoom() != pUser->GetEventRoom()
		|| GetZoneID() == ZONE_CHAOS_DUNGEON 
		|| (isQuestEventUser() && GetZoneID() >= ZONE_STONE1 && GetZoneID() <= ZONE_STONE3)
		|| (g_pMain->m_byBattleOpen == SIEGE_BATTLE && GetZoneID() == ZONE_DELOS && GetClanID() != pUser->GetClanID()))
	{
		errorCode = -3;
		goto fail_return;
	}
...
void CUser::SendPartyBBSNeeded(uint16 page_index, uint8 bType)
{
	...
	int j = 0;
	bool bIsNeutralZone = (GetZoneID() >= ZONE_MORADON && GetZoneID() <= ZONE_MORADON5 || GetZoneID() == ZONE_FORGOTTEN_TEMPLE);
