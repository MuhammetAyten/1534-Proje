...
/*	bool LoadItemExchangeExpTable();
	...
	bool LoadMiningItemTable();
	bool LoadFishingItemTable();
	...
	void AliveUserCheck();
	void TerminationFinish(uint16 EventRoom = 0);
	...
	void DeleteAllNpcList(CAISocket *pSock = nullptr);
	CNpc*  FindNpcInZone(uint16 sPid, uint8 byZone);
	CNpc* UpdateNpc();
	void AktiveTempleEventTimer();
	...
	uint8 TempleQuestEventGetRoomUsers(uint16 nEventRoom = 0);
	...
	//Quest Event System
	void AddQuestEventUser(CUser * pUser);
	void RemoveQuestEventUser(CUser * pUser);
	void UpdateQuestEventUser(CUser * pUser, uint16 nEventRoom = 0);
	void SetQuestEventUser(CUser *pUser);
	void TempleQuestEventTeleportUsers(CUser *pUser);
	...
	void SpawnEventNpc(uint16 sSid, bool bIsMonster, uint8 byZone, float fX, float fY, float fZ, uint8 byDirection = 0, uint16 sCount = 1, uint16 sRadius = 0, uint16 sDuration = 0, uint8 nation = 0, int16 socketID = -1, uint16 nEventRoom = 0); // DuslerSokagi Ekledi YNG Geliþtirdi..
	...
	MiningItemArray						m_MiningItemArray;
	FishingItemArray					m_FishingItemArray;
	...
	TempleQuestEventUserArray			m_TempleQuestEventUserArray;
	...
	UserSealItemArray					m_UserSealItemArray;
	...
	uint8 m_ReloadKnightAndUserRanksMinute, m_sGetSecond;
	...
	int16	m_byCastleSýegeWarTime;
	int32	m_byCastleSýegeWarMonumentTime;
	int32	m_byBattleSiegeWarStartTime;
	int32	m_byCastleSiegeWarMonumentTime;
	int32   m_byCastleSiegeWarTime;
	...
	std::string m_sCastleBattleZoneOpenDays;
	uint8	m_nBattleZoneOpenHourStart[WAR_TIME_COUNT], m_nBattlezoneOpenWarZone[WAR_ZONE_COUNT], m_nCastleBattleZoneOpenHourStart[WAR_TIME_COUNT];

	std::string m_strKarusCaptain, m_strElmoradCaptain;

	uint8   m_nBorderDefenseWarTime[BORDER_DEFENSE_WAR_EVENT_COUNT], m_nChaosExpansionTime[CHAOS_EVENT_COUNT], m_nJuraidMountionTime[JURAD_MOUNTAIN_EVENT_COUNT];
	...
	void EventRemainingTime(CUser *pUser = nullptr, uint8 ZoneID = 0);
	void CastleSiegeWarFlag(CUser *pUser = nullptr, bool SendZonePacket = false);
	...
	void WritePUSLogFile(std::string & logMessage);
