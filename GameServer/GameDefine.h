...
struct	_EXCHANGE_ITEM
{
	...
	INLINE bool isUserSeal() { return nUserSeal > 0; }
};
...
struct _TEMPLE_QUEST_EVENT_USER
{
	uint16 m_socketID;
	uint16 m_bEventRoom;
};
...
struct _EVENT_STATUS
{
	...
	uint16 m_sBorderMonumentTimer[MAX_TEMPLE_EVENT_ROOM];
	uint8 m_sBorderMonumentNation[MAX_TEMPLE_EVENT_ROOM];
	uint8 m_sAllEventFinishTimer[MAX_TEMPLE_EVENT_ROOM];
	bool  m_MonumentFinish[MAX_TEMPLE_EVENT_ROOM];
	// Juraid
	uint8 isSummonsBridgeMinute[MAX_TEMPLE_EVENT_ROOM];
	uint8 isMonsterKilledCount[MAX_TEMPLE_EVENT_ROOM];
	uint8 isMonsterAddKilledCount[MAX_TEMPLE_EVENT_ROOM];

	bool isSummonsBridge[MAX_TEMPLE_EVENT_ROOM];
	bool isDevaFlag[MAX_TEMPLE_EVENT_ROOM];
	bool isDevaControl[MAX_TEMPLE_EVENT_ROOM];
	// Juraid Finish

	bool isAttackable[MAX_TEMPLE_EVENT_ROOM];
	bool isActive;
};
