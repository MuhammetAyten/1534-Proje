...
void CUser::ReqLetterSend(Packet & pkt)
{
	...
	uint32 nItemID = 0, nCoins = 0, nCoinRequirement = 1000, nUserSeal = 0;
	uint8 bType, bSrcPos;
	int8 bResult = 1;
	int64 Serial = 0;

	if (isMerchanting() || isTrading() || !isInGame())
	{
		bResult = -1;
		goto send_packet;
	}
	...
	if (pItem != nullptr)
	{
		if (pItem->nNum == nItemID && pItem->nSerialNum == Serial && pItem->nUserSeal == nUserSeal)
...
void CUser::ReqLetterGetItem(Packet & pkt)
{
	Packet result(WIZ_SHOPPING_MALL, uint8(STORE_LETTER));
	uint64 nSerialNum = 0;
	uint32 nLetterID = pkt.read<uint32>(), nItemID = 0, nCoins = 0, nUserSeal = 0;
	uint16 sCount = 0, sDurability = 0;
	int8 bResult = g_DBAgent.GetItemFromLetter(m_strUserID, nLetterID, nItemID, sCount, sDurability, nCoins, nSerialNum, nUserSeal);
	int pos = -1;
