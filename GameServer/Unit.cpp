	...
	if (isIncapacitated()
		// or if our target is in a state in which
			// they should not be allowed to be attacked
				|| pTarget->isDead()
				|| pTarget->isMonsterTransformation()
				|| pTarget->isBlinking())
				return false;
...
void KOMap::SetZoneAttributes(int zoneNumber)
{
	...
	case ZONE_STONE3:
		m_zoneType = ZoneAbilityNeutral;
		m_zoneFlags = ZF_TRADE_OTHER_NATION | ZF_FRIENDLY_NPCS;
		break;
	case ZONE_PRISON:
		m_zoneType = ZoneAbilityPVPNeutralNPCs;
		m_zoneFlags = ZF_ATTACK_OTHER_NATION | ZF_FRIENDLY_NPCS;
...
bool CNpc::isHostileTo(Unit * pTarget)
{
	...
	{
		if (GetProtoID() > 1100 && GetProtoID() <= 2203 || GetProtoID() > 2301 && GetProtoID() <= 2306)
			return true;
	}

	if (pTarget->isPlayer())
	{
		if (GetZoneID() == ZONE_DELOS && (GetType() == NPC_DESTROYED_ARTIFACT || isGate()))
		{
			...
			if(pKnightSiege->sMasterKnights > 0 && TO_USER(pTarget)->GetClanID() == pKnightSiege->sMasterKnights)
				return false;

			CKnights *pKnights = g_pMain->GetClanPtr(TO_USER(pTarget)->GetClanID());
			CKnights *pKnightsDelosLord = g_pMain->GetClanPtr(pKnightSiege->sMasterKnights);
			if(pKnights != nullptr 
				&& pKnightsDelosLord != nullptr 
				&& pKnights->isInAlliance()
				&& pKnightsDelosLord->isInAlliance()
				&& (pKnights->GetAllianceID() == pKnightsDelosLord->GetAllianceID()))
				return false;
			else if(g_pMain->m_byBattleOpen != SIEGE_BATTLE)
				return false;
...
bool CUser::isHostileTo(Unit * pTarget)
{
...
#if defined(GAMESERVER)
	if (g_pMain->m_byBattleOpen == SIEGE_BATTLE && GetZoneID() == ZONE_DELOS && isInClan())
	{
		if (!TO_USER(pTarget)->isInClan())
			return false;
		...
		{		
			...
			if(pKnightSiege->sMasterKnights == pKnightsSource->GetID()
				|| (pKnightsSource->GetAllianceID() > 0 && pKnightSiege->sMasterKnights == pKnightsSource->GetAllianceID()))
			{
				if(pKnightsTarget->GetAllianceID() == 0 && pKnightsTarget->GetID() != pKnightsSource->GetID())
					return true;

				if(pKnightsTarget->GetAllianceID() > 0 && pKnightsTarget->GetAllianceID() != pKnightsSource->GetAllianceID())
					return true;
			}
			else
			{
				if(pKnightSiege->sMasterKnights == pKnightsTarget->GetID()
					|| (pKnightsTarget->GetAllianceID() > 0 && pKnightsTarget->GetAllianceID() == pKnightSiege->sMasterKnights))
					return true;

				if(pKnightSiege->sMasterKnights == 0
					&& (pKnightsTarget->GetAllianceID() == 0 && pKnightsTarget->GetID() != pKnightsSource->GetID()))
					return true;

				if(pKnightSiege->sMasterKnights == 0
					&& (pKnightsTarget->GetAllianceID() > 0 && pKnightsTarget->GetAllianceID() != pKnightsSource->GetAllianceID()))
					return true;
			}
		}
		return false;
	...
	if (GetEventRoom() == pTarget->GetEventRoom() 
		&& ChaosTempleEventZone())
		return true;

	if (GetNation() != pTarget->GetNation() 
		&& GetEventRoom() == pTarget->GetEventRoom()
		&& isInTempleQuestEventZone())
		return true;
...
bool CUser::isInSafetyArea()
{
	...
	case ZONE_BATTLE3:
		if (GetNation() == KARUS)
			return ((GetX() < 800.0f && GetX() > 850.0f) && ((GetZ() < 950.0f && GetZ() > 900.0f)));
		else if (GetNation() == ELMORAD)
			return ((GetX() < 200.0f && GetX() > 4150.0f) && ((GetZ() < 50.0f && GetZ() > 95.0f)));

	case ZONE_BATTLE4:
		if (GetNation() == KARUS)
			return ((GetX() < 249.0f && GetX() > 184.0f) && ((GetZ() < 251.0f && GetZ() > 213.0f)));
		else if (GetNation() == ELMORAD)
			return ((GetX() < 832.0f && GetX() > 771.0f) && ((GetZ() < 798.0f && GetZ() > 762.0f)));
