...
public:
	...
	bool LoadKnightsAllMembers(uint16 sClanID, _KNIGHTS_USER* p);
	...
	bool LoadAllianceInfo(uint16 sClanID, uint16 & sSubAllianceKnights, uint16 & sMercenaryClan_1, uint16 & sMercenaryClan_2);
	bool LoadCapeId(uint16 sClanID, uint16 & m_sCape);
	...
	void UpdateAllianceNotice(uint16 sClanID, std::string & strAllianceNotice);
	NameChangeOpcode UpdateCharacterName(std::string & strAccountID, std::string & strUserID, std::string & strNewUserID);
	ClanNameChangeOpcode UpdateCharacterClanName(std::string & strClanID, std::string & strNewClanID);
	...
	int8 GetItemFromLetter(std::string & strCharID, uint32 nLetterID, uint32 & nItemID, uint16 & sCount, uint16 & sDurability, uint32 & nCoins, uint64 & nSerialNum, uint32 & nUserSeal);
	...
	bool LoadSealInfo(std::string & CharID, uint8 & blevel, uint16 & bExp, uint8 & bRace, uint16 & bJob, uint8 & STR, uint8 & STA, uint8 & DEX, uint8 & INT, uint8 & CHA, uint32 & nGold, ByteBuffer & strSkill, ByteBuffer & strItem);
	bool CharacterSealSave(std::string strCharID, uint64 bSerial, uint32 SealSerial, uint8 bRaceSatis, uint16 sClass, uint8 bLevel, uint16 pExp, uint8 & STR, uint8 & STA, uint8 & DEX, uint8 & INT, uint8 & CHA, uint32 & nGold, ByteBuffer & bstrSkill, ByteBuffer & bstrItem);
	bool CharacterSealDelete(std::string CharID);
	void UpdateAccountChar(std::string AccountID, std::string strChardID, uint8 bRent = 0);
	void LoadSealPasswd(std::string AccountID, std::string & strPasswd);
