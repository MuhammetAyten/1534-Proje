...
void CGameServerDlg::GetTimeFromIni()
{
	...
	ini.GetString("ODBC", "GAME_UID", "Alpha", m_strGameUID, false);
	ini.GetString("ODBC", "GAME_PWD", "Alpha1534", m_strGamePWD, false);

	m_bMarsEnabled = ini.GetBool("ODBC", "GAME_MARS", false);

	ini.GetString("ODBC", "ACCOUNT_DSN", "KN_online", m_strAccountDSN, false);
	ini.GetString("ODBC", "ACCOUNT_UID", "Alpha", m_strAccountUID, false);
	ini.GetString("ODBC", "ACCOUNT_PWD", "Alpha1534", m_strAccountPWD, false);
	...
	ini.GetString("CASTLE_BATTLE", "DAYS", "2,0", m_sCastleBattleZoneOpenDays, false);

	for (int i = 0; i < WAR_TIME_COUNT; i++)
		m_nCastleBattleZoneOpenHourStart[i] = ini.GetInt("CASTLE_BATTLE",string_format("START_TIME%d",i).c_str(), (i+1) * 8);
	...
	m_byMaxLevel = ini.GetInt("SETTINGS","GAMEMAXLEVEL", 80);
	...
	m_Grade2 = ini.GetInt("CLAN_GRADE","GRADE2", 340000);
	m_Grade3 = ini.GetInt("CLAN_GRADE","GRADE3", 50000);
	m_Grade4 = ini.GetInt("CLAN_GRADE","GRADE4", 25000);

	m_upgrade2 = ini.GetInt("UPGRADE_RATES","KARIVDIS_RATE", 100);
	m_upgrade1 = ini.GetInt("UPGRADE_RATES","TRINA_RATE", 100);
	...
	for (int i = 0; i < BORDER_DEFENSE_WAR_EVENT_COUNT; i++)
		m_nBorderDefenseWarTime[i] = ini.GetInt("BORDER_DEFENSE_WAR",string_format("START_TIME%d",i+1).c_str(), 0);
#if 0
	for (int i = 0; i < CHAOS_EVENT_COUNT; i++)
		m_nChaosExpansionTime[i] = ini.GetInt("CHAOS_EXPANSION",string_format("START_TIME%d",i+1).c_str(), 0);
#endif
	for (int i = 0; i < JURAD_MOUNTAIN_EVENT_COUNT; i++)
		m_nJuraidMountionTime[i] = ini.GetInt("JURAID_MOUNTAIN",string_format("START_TIME%d",i+1).c_str(), 0);
	...
	//Quest Event System
	pQuestEvent.ActiveEvent = -1;
	pQuestEvent.StartTime = (uint32)UNIXTIME;
	pQuestEvent.m_eEventTime = 0;
	pQuestEvent.LastEventRoom = 1;
	pQuestEvent.AllUserCount = 0;
	pQuestEvent.ElMoradUserCount = 0;
	pQuestEvent.KarusUserCount = 0;
	pQuestEvent.isActive = false;
	pQuestEvent.isAttackable = false;	

	//Event System
	pTempleEvent.ActiveEvent = -1;
	pTempleEvent.StartTime = (uint32)UNIXTIME;
	pTempleEvent.ZoneID = 0;
	pTempleEvent.m_eEventTime = 0;
	...
	memset(pTempleEvent.m_sBorderMonumentTimer , 0 , sizeof(pTempleEvent.m_sBorderMonumentTimer));
	memset(pTempleEvent.m_sBorderMonumentNation , 0 , sizeof(pTempleEvent.m_sBorderMonumentNation));
	memset(pTempleEvent.m_sAllEventFinishTimer , 0 , sizeof(pTempleEvent.m_sAllEventFinishTimer));
	memset(pTempleEvent.isAttackable , false, sizeof(pTempleEvent.isAttackable));
	memset(pTempleEvent.m_MonumentFinish , false, sizeof(pTempleEvent.m_MonumentFinish));

	//Juraid Stard
	memset(pTempleEvent.isDevaControl , false , sizeof(pTempleEvent.isDevaControl));
	memset(pTempleEvent.isDevaFlag , false , sizeof(pTempleEvent.isDevaFlag));
	memset(pTempleEvent.isSummonsBridgeMinute , 0 , sizeof(pTempleEvent.isSummonsBridgeMinute));
	memset(pTempleEvent.isMonsterKilledCount , 0 , sizeof(pTempleEvent.isMonsterKilledCount));
	memset(pTempleEvent.isMonsterAddKilledCount , 0 , sizeof(pTempleEvent.isMonsterAddKilledCount));
	memset(pTempleEvent.isSummonsBridge , false , sizeof(pTempleEvent.isSummonsBridge));
	//Juraid Stop
...
*/
void CGameServerDlg::SpawnEventNpc(uint16 sSid, bool bIsMonster, uint8 byZone, float fX, float fY, float fZ, uint8 byDirection, uint16 sCount /*= 1*/, uint16 sRadius /*= 0*/, uint16 sDuration /*= 0*/, uint8 nation /*= 0*/,int16 socketID /*= -1*/, uint16 nEventRoom)
{
...
uint32 CGameServerDlg::Timer_CheckGameEvents(void * lpParam)
{
	while (g_bRunning)
	{
		...
		g_pMain->AktiveTempleEventTimer();
											 ...
											 uint8 nation, uint8 seekingPartyOptions, uint16 nEventRoom)
{
	...
	foreach (itr, sessMap)
	{
		...
		if (pUser->GetZoneID() >= ZONE_MORADON && pUser->GetZoneID() <= ZONE_MORADON5
...
void CGameServerDlg::UpdateGameTime()
{
	...
	if (m_sSec != now.GetSecond())
	{
		...
		if (m_sGetSecond == RELOAD_GAME_UPDATE)
		{		
			m_sGetSecond = 0;
		...
		//Monster update at the moment Skill System
		foreach_stlmap(itr, m_arNpcArray)
		{
			CNpc * pNpc = TO_NPC(itr->second);
	...
	if (m_sMin != now.GetMinute())
	{
			...
			&& pTempleEvent.ZoneID == ZONE_JURAID_MOUNTAIN)
		{
			...
			{
				pTempleEvent.isSummonsBridgeMinute[i+1]++;

				if (pTempleEvent.isSummonsBridgeMinute[i+1] == RELOAD_GAME_UPDATE && pTempleEvent.isSummonsBridge[i+1])
				{
					pTempleEvent.isSummonsBridge[i+1] = false;
					pTempleEvent.isSummonsBridgeMinute[i+1] = 0;
					break;
				}

				if (pTempleEvent.isSummonsBridgeMinute[i+1] == RELOAD_GAME_UPDATE && !pTempleEvent.isSummonsBridge[i+1])
				{
					pTempleEvent.isSummonsBridge[i+1] = true;
					pTempleEvent.isSummonsBridgeMinute[i+1] = 0;
					break;
				}
			}
...
void CGameServerDlg::GetRegionNpcIn(C3DMap *pMap, uint16 region_x, uint16 region_z, Packet & pkt, uint16 & t_count, uint16 nEventRoom, CUser* pSendUser)
{
	...
	foreach (itr, pRegion->m_RegionNpcArray)
	{
		...
		if ((pNpc->GetProtoID() == 541 && pNpc->GetType() == NPC_DESTROYED_ARTIFACT && pNpc->GetZoneID() == ZONE_DELOS) 
			|| (pNpc->GetType() == NPC_GATE	&& pNpc->GetZoneID() == ZONE_DELOS && (pNpc->GetProtoID() == 561 || pNpc->GetProtoID() == 562 || pNpc->GetProtoID() == 563)))
		{
...
void CGameServerDlg::GetRegionNpcList(C3DMap *pMap, uint16 region_x, uint16 region_z, Packet & pkt, uint16 & t_count, uint16 nEventRoom)
{
	...
	foreach (itr, pRegion->m_RegionNpcArray)
	{
		...
		if (nEventRoom > 0 && nEventRoom != pNpc->GetEventRoom() && pNpc->GetProtoID() != SAW_BLADE_SSID)
			continue;
...
void CGameServerDlg::DeleteAllNpcList(CAISocket *pSock)
{
	...
	if (pSock != nullptr)
		printf("AIServer disconnected - %s\n", pSock->GetRemoteIP().c_str());

	printf("*** DeleteAllNpcList - Start *** \n");
	...
	printf("*** DeleteAllNpcList - End *** \n");

	printf("[Monster Point Delete]\n");
}
...
void CGameServerDlg::BattleZoneOpenTimer()
{
	...
	if (!isWarOpen())
	{
		std::list<std::string> nLunarBattleWar = StrSplit(m_sBattleZoneOpenDays, ",");
		uint8 nLunarBattleWarDaySize = (uint8) nLunarBattleWar.size();
		if (nLunarBattleWarDaySize > 0)
		{
			uint8 nLunarBattleWarDay = 0;
			for (int i = 0; i < nLunarBattleWarDaySize; i++)
			{
				nLunarBattleWarDay = atoi(nLunarBattleWar.front().c_str());
				if (nLunarBattleWarDay == nWeekDay)
				{
					for (int x = 0; x < WAR_TIME_COUNT; x++)
					{
						if ((m_nBattleZoneOpenHourStart[x] - (1)) == nHour && nMin == 55 && nSec == 0)
						{
							m_byBattleNoticeTime = 5;
							Announcement(IDS_PRE_BATTLE_ANNOUNCEMENT);
						}
						else if ((m_nBattleZoneOpenHourStart[x] - (1)) == nHour && nMin == 56 && nSec == 0)
						{
							m_byBattleNoticeTime = 4;
							Announcement(IDS_PRE_BATTLE_ANNOUNCEMENT);
						}
						else if ((m_nBattleZoneOpenHourStart[x] - (1)) == nHour && nMin == 57 && nSec == 0)
						{
							m_byBattleNoticeTime = 3;
							Announcement(IDS_PRE_BATTLE_ANNOUNCEMENT);
						}
						else if ((m_nBattleZoneOpenHourStart[x] - (1)) == nHour && nMin == 58 && nSec == 0)
						{
							m_byBattleNoticeTime = 2;
							Announcement(IDS_PRE_BATTLE_ANNOUNCEMENT);
						}
						else if ((m_nBattleZoneOpenHourStart[x] - (1)) == nHour && nMin == 59 && nSec == 0)
						{
							m_byBattleNoticeTime = 1;
							Announcement(IDS_PRE_BATTLE_ANNOUNCEMENT);
						}
						if (m_nBattleZoneOpenHourStart[x] == nHour && nMin == 0 && nSec == 0)
							BattleZoneOpen(BATTLEZONE_OPEN, m_nBattlezoneOpenWarZone[x]);
					}
				}
				nLunarBattleWar.pop_front();
			}
		}
		std::list<std::string> nBattleSiegeWar = StrSplit(m_sCastleBattleZoneOpenDays, ",");
		uint8 nCastleBattleDaySize = (uint8) nBattleSiegeWar.size();
		if (nCastleBattleDaySize > 0)
		{
			uint8 nCastleBattleDay = 0;
			for (int i = 0; i < nCastleBattleDaySize; i++)
			{
				nCastleBattleDay = atoi(nBattleSiegeWar.front().c_str());
				if (nCastleBattleDay == nWeekDay)
				{
					for (int x = 0; x < WAR_TIME_COUNT; x++)
					{
						if (m_nCastleBattleZoneOpenHourStart[x] == nHour && nMin == 0 && nSec == 0)
							CastleSiegeWarZoneOpen(SIEGE_BATTLE);
					}
				}
				nBattleSiegeWar.pop_front();
			}
		}
	}
	else if (m_byBattleOpen == NATION_BATTLE)	
	{
		...
		if (m_bVictory == 0)
		{
			...
			else if (WarElapsedTime == (m_byBattleTime / 8) && nBattleZone == ZONE_BATTLE4 ||
				WarElapsedTime == (m_byBattleTime / 8) && nBattleZone == ZONE_BATTLE5) // War half time + 15 minutes for Nereid's , Zipang Island.
			{
				if (nBattleZone == ZONE_BATTLE4 || nBattleZone == ZONE_BATTLE5) // Nereid's Island
				{
	...
	else if (m_byBattleOpen == SIEGE_BATTLE)
	{
		if (m_byBattleSiegeWarStartTime)
		{
			if (m_byBattleSiegeWarStartTime == (5 * MINUTE))
			{
				m_SiegeWarNoticeTime = 5;
				Announcement(IDS_SIEGE_WAR_TIME_NOTICE);
				KickOutZoneUsers(ZONE_DELOS, ZONE_MORADON);
			}
			else if (m_byBattleSiegeWarStartTime == (4 * MINUTE))
			{
				m_SiegeWarNoticeTime = 4;
				Announcement(IDS_SIEGE_WAR_TIME_NOTICE);
				KickOutZoneUsers(ZONE_DELOS, ZONE_MORADON);
			}
			else if (m_byBattleSiegeWarStartTime == (3 * MINUTE))
			{
		...
		if (!m_byBattleSiegeWarStartTime && !m_byBattleSiegeWarTeleport)
		{
		...
		if (!m_byBattleSiegeWarStartTime)
		{
			...
			if (m_byCastleSiegeWarMonumentTime && m_byBattleSiegeWarMomument)
				m_byCastleSiegeWarMonumentTime--;

			if (m_byCastleSiegeWarTime)
				m_byCastleSiegeWarTime--;

			if (m_sBattleTimeDelay == (40 * MINUTE))
			{
				m_SiegeWarNoticeTime = 20;
				Announcement(IDS_SIEGE_WAR_TIME_ANNOUNCEMENT);
			}
			...
			if (m_byCastleSiegeWarMonumentTime == 10 * MINUTE && m_byBattleSiegeWarMomument)
			{
				m_SiegeWarNoticeTime = 10;
				Announcement(IDS_SIEGE_WAR_CLAN_ANNOUNCEMENT);
			}
			else if (m_byCastleSiegeWarMonumentTime == 8 * MINUTE && m_byBattleSiegeWarMomument)
			{
				m_SiegeWarNoticeTime = 8;
				Announcement(IDS_SIEGE_WAR_CLAN_ANNOUNCEMENT);
			}
			else if (m_byCastleSiegeWarMonumentTime == 6 * MINUTE && m_byBattleSiegeWarMomument)
			{
				m_SiegeWarNoticeTime = 6;
				Announcement(IDS_SIEGE_WAR_CLAN_ANNOUNCEMENT);
			}
			else if (m_byCastleSiegeWarMonumentTime == 4 * MINUTE && m_byBattleSiegeWarMomument)
			{
				m_SiegeWarNoticeTime = 4;
				Announcement(IDS_SIEGE_WAR_CLAN_ANNOUNCEMENT);
			}
			else if (m_byCastleSiegeWarMonumentTime == 2 * MINUTE && m_byBattleSiegeWarMomument)
			{
				m_SiegeWarNoticeTime = 2;
				Announcement(IDS_SIEGE_WAR_CLAN_ANNOUNCEMENT);
			}
			...
			if(!m_byCastleSiegeWarTime ||
				(!m_byCastleSiegeWarMonumentTime && m_byBattleSiegeWarMomument))
				BattleZoneClose();
		}
	}
	if (m_byBanishFlag)
	...
	if (m_bySiegeBanishFlag)
	{
		...
		if (m_sBanishDelay == (m_byBattleTime / 360))
			...
			m_bySiegeBanishFlag = false;
			m_sBanishDelay = 0;
			BanishSiegewarfareLosers();
		}
	}
}
...
void CGameServerDlg::BattleZoneResult(uint8 nation)
{
	...
	foreach (itr, sessMap)
	{
		...
		Packet result(WIZ_BATTLE_EVENT, uint8(1));
...
{
	if ((nType == SIEGE_BATTLE || nType == CLAN_BATTLE/**/) && !g_pMain->isWarOpen())
	{
		...
		m_byCastleSiegeWarTime = m_byCastleSiegeWarMonumentTime = 1 * HOUR;
		m_SiegeWarNoticeTime = 10;

		if (nType == SIEGE_BATTLE)
			Announcement(IDS_SIEGE_WAR_TIME_NOTICE);	// Send an announcement out that the battlezone is open/closed.
...
void CGameServerDlg::BattleEventGiveItem(int nType)
{
	...
	foreach (itr, sessMap)
	{
		...
		if (nType == BATTLEZONE_OPEN && m_byBattleZoneType == BATTLEZONE_OPEN)
			result << uint8(GOLDSHELL) << uint8(true) << pUser->GetSocketID();
		else if (nType = BATTLEZONE_CLOSE && m_byBattleZoneType == BATTLEZONE_OPEN)
...
{
	if (m_byBattleOpen == SIEGE_BATTLE && isWarOpen())
	{
...
* @brief	Kicks invaders out of the invaded nation after a war
**/
void CGameServerDlg::BanishSiegewarfareLosers()
{
	SessionMap sessMap = g_pMain->m_socketMgr.GetActiveSessionMap();
	foreach (itr, sessMap)
	{
...
void CGameServerDlg::ResetBattleZone()
{
	...
	m_byCastleSiegeWarMonumentTime = 0;
	m_byCastleSiegeWarTime = 0;
...
void CGameServerDlg::AktiveTempleEventTimer()
{
	...
	else if (m_bForgettenTempleIsActive)
	{
		...
		else if (m_bForgettenTempleSummonMonsters)
		{
			foreach_stlmap_nolock (itr, m_MonsterChallengeSummonListArray)
			{
				...
				{
					SpawnEventNpc(itr->second->sSid,true,ZONE_FORGOTTEN_TEMPLE,(float)itr->second->sPosX,0,(float)itr->second->sPosZ,0,itr->second->sCount,itr->second->bRange);
...
void CGameServerDlg::TempleEventTimer()
{
	...
	if (pTempleEvent.ActiveEvent == -1)
	{
		for (int i = 0; i < BORDER_DEFENSE_WAR_EVENT_COUNT; i++)
		{
			...
			{
				SendAnnouncement("The National Defence Combat Border Defence War Start.");
		...
		for (int i = 0; i < CHAOS_EVENT_COUNT; i++)
		{
			if (nHour == m_nChaosExpansionTime[i] && nMinute == 0)
			{
		...
		for (int i = 0; i < JURAD_MOUNTAIN_EVENT_COUNT; i++)
		{
			if (nHour == m_nJuraidMountionTime[i] && nMinute == 0) 
			{
	...
	else if (pTempleEvent.ActiveEvent != -1)
	{
		if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR)
		{
			...
			{
				if(pTempleEvent.m_sBorderMonumentNation[i] == 0)
					continue;

				if (pTempleEvent.m_sBorderMonumentTimer[i] > 0)
					pTempleEvent.m_sBorderMonumentTimer[i]--;

				if (pTempleEvent.m_MonumentFinish[i] && pTempleEvent.m_sBorderMonumentTimer[i] <= 0)
				{
					pTempleEvent.m_sAllEventFinishTimer[i]++;

					if (pTempleEvent.m_MonumentFinish[i] && pTempleEvent.isAttackable[i])
					{
						pTempleEvent.isAttackable[i] = false;
						TerminationFinish(i);
						break;
					}
					if (pTempleEvent.m_MonumentFinish[i] && pTempleEvent.m_sAllEventFinishTimer[i] == TEMPLE_TERMINITION_TIME_FINISH && !pTempleEvent.isAttackable[i])
					{
						pTempleEvent.m_MonumentFinish[i] = false;
						pTempleEvent.m_sAllEventFinishTimer[i] = 0;
						pTempleEvent.m_sBorderMonumentNation[i] = 0;
						TempleEventFinish(i);
						break;
					}break;
				}
			...
			for(int i = 0; i < BORDER_DEFENSE_WAR_EVENT_COUNT; i++)
			{
				...
				{
					for(int i = 0; i < MAX_TEMPLE_EVENT_ROOM; i++)
						pTempleEvent.isAttackable[i] = true;
					break;
				}
				else if (nHour == m_nBorderDefenseWarTime[i] && nMinute == 40 && nSeconds == 0)   
				{
					foreach_stlmap (itr, m_TempleEventUserArray)
					{
						...
						TerminationFinish(pUser->GetEventRoom());						
					}
					break;
				}
		...
		if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_CHAOS)
		{
			for(int i = 0; i < CHAOS_EVENT_COUNT; i++)
			{
				if (nHour == m_nChaosExpansionTime[i] && nMinute == 10 && nSeconds == 0) 
				{
					m_nTempleEventRemainSeconds = 0;
					pTempleEvent.LastEventRoom = 1;
					pTempleEvent.isActive = true;
				...
				else if (nHour == m_nChaosExpansionTime[i] && nMinute == 11 && nSeconds == 0) 
				{
					for(int i = 0; i < MAX_TEMPLE_EVENT_ROOM; i++)
						pTempleEvent.isAttackable[i] = true;
					break;
				}
				else if (nHour == m_nChaosExpansionTime[i] && nMinute == 30 && nSeconds == 0) 
				{
					foreach_stlmap (itr, m_TempleEventUserArray)
					{
						CUser * pUser = GetUserPtr(itr->second->m_socketID);
				...
				else if (nHour == m_nChaosExpansionTime[i] && nMinute == 30 && nSeconds == 20)
				{
					foreach_stlmap (itr, m_TempleEventUserArray)
					{
						CUser * pUser = GetUserPtr(itr->second->m_socketID);
				...
				else if (nHour == m_nChaosExpansionTime[i] && nMinute == 30 && nSeconds == 30)
				{
		...
		if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN)
		{
			for(int i = 0; i < MAX_TEMPLE_EVENT_ROOM; i++)
			{
				...
				if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN && pTempleEvent.isDevaControl[i])
				{
					pTempleEvent.m_sAllEventFinishTimer[i]++;

					if (pTempleEvent.isDevaFlag[i] && pTempleEvent.isAttackable[i])
					{
						pTempleEvent.isAttackable[i] = false;
					...
					if (pTempleEvent.isDevaControl[i] && pTempleEvent.m_sAllEventFinishTimer[i] == TEMPLE_TERMINITION_TIME_FINISH && !pTempleEvent.isAttackable[i])
					{
			...
			for(int i = 0; i < JURAD_MOUNTAIN_EVENT_COUNT; i++)
			{
				if (nHour == m_nJuraidMountionTime[i] && nMinute == 10 && nSeconds == 0)
				{
					m_nTempleEventRemainSeconds = 0; 
					pTempleEvent.LastEventRoom = 1;
					pTempleEvent.isActive = true;
				...
				else if (nHour == m_nJuraidMountionTime[i] && nMinute == 11 && nSeconds == 0)   
				{
					for(int i = 0; i < MAX_TEMPLE_EVENT_ROOM; i++)
						pTempleEvent.isAttackable[i] = true;
					break;
				}
				else if (nHour == m_nJuraidMountionTime[i] && nMinute == 50 && nSeconds == 0)   
				{
					foreach_stlmap (itr, m_TempleEventUserArray)
					{
						CUser * pUser = GetUserPtr(itr->second->m_socketID);
						...
						for(int t = 0; t < MAX_TEMPLE_EVENT_ROOM; t++)
							pTempleEvent.isAttackable[t] = false;

						TerminationFinish(pUser->GetEventRoom());
					}
					break;
				}
				else if (nHour == m_nJuraidMountionTime[i] && nMinute == 50 && nSeconds == 20) 
				{
					foreach_stlmap (itr, m_TempleEventUserArray)
					{
						CUser * pUser = GetUserPtr(itr->second->m_socketID);
				...
				else if (nHour == m_nJuraidMountionTime[i] && nMinute == 50 && nSeconds == 30)
				{
					printf("JURAID MOINTION TUM KANALLAR KAPATILDI !\n");
					TempleEventReset();
					break;
				}
			}
...
void CGameServerDlg::TempleEventTeleportUsers()
{
	...
	case TEMPLE_EVENT_BORDER_DEFENCE_WAR:
			for(int i = 0; i < MAX_TEMPLE_EVENT_ROOM; i++)
				pTempleEvent.isAttackable[i] = true;
			pTempleEvent.m_eEventTime = TEMPLE_EVENT_BORDER_TIME;
		break;
#if 0
	case TEMPLE_EVENT_CHAOS:
			for(int i = 0; i < MAX_TEMPLE_EVENT_ROOM; i++)
				pTempleEvent.isAttackable[i] = false;
			pTempleEvent.m_eEventTime = TEMPLE_EVENT_CHAOS_TIME;
		break;
#endif
	case TEMPLE_EVENT_JURAD_MOUNTAIN:
			for(int i = 0; i < MAX_TEMPLE_EVENT_ROOM; i++)
				pTempleEvent.isAttackable[i] = false;
			pTempleEvent.m_eEventTime = TEMPLE_EVENT_JURAID_TIME;
		break;
	}
...
#endif
		if (pTempleEvent.ZoneID == ZONE_JURAID_MOUNTAIN)
...
uint8 CGameServerDlg::TempleQuestEventGetRoomUsers(uint16 nEventRoom) 
{
	uint8 nEventRoomUserCount = 0;

	foreach_stlmap (itr, m_TempleQuestEventUserArray)
	{
		if (itr->second->m_bEventRoom == nEventRoom)
			nEventRoomUserCount++;
	}
...
{
	uint16 sFamilly[MAX_TEMPLE_EVENT_ROOM];
#if 0
	if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_CHAOS)
	{
		for (int i = 0; i < g_pMain->pTempleEvent.LastEventRoom; i++)
		{
			foreach_stlmap_nolock (itr, m_MonsterRespawnListInformationArray)
			{
				if (itr->second->ZoneID == ZONE_CHAOS_DUNGEON)
					SpawnEventNpc(itr->second->sSid,itr->second->bType == 0 ? true : false,itr->second->ZoneID,itr->second->X,itr->second->Y,itr->second->Z,itr->second->byDirection,itr->second->sCount,itr->second->bRadius,60*MINUTE,0,-1,i+1/* EventRoom */);
			}
		}
	}
#endif
	...
	else if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN)
	{
		for (int i = 0; i < g_pMain->pTempleEvent.LastEventRoom; i++)
		{
			sFamilly[i+1] = myrand( 21, 28);

			foreach_stlmap_nolock (itr, m_JuraidMountionListInformationArray)
			{
				if (itr->second->ZoneID == pTempleEvent.ZoneID 
					&& itr->second->Family == sFamilly[i+1])
					SpawnEventNpc(itr->second->sSid,itr->second->bType == 0 ? true : false,itr->second->ZoneID,itr->second->X,itr->second->Y,itr->second->Z,itr->second->byDirection,itr->second->sCount,itr->second->bRadius,60*MINUTE,0,-1,i+1/* EventRoom */);
			}
		}
	}
...
void CGameServerDlg::TempleEventFinish(uint16 m_sEventRoom, uint8 m_bNation)
{
	foreach_stlmap (itr, m_TempleEventUserArray)
	{
		...
		if (pRankInfo)
		{
			...
			{
				if (m_sEventRoom == pUser->GetEventRoom())
				{
					...
					else if (pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] > pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] && pUser->GetNation() == ELMORAD) 
						pUser->GiveItem(RED_TREASURE_CHEST,1);
					else if (pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] > pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] && pUser->GetNation() == KARUS) 
						pUser->GiveItem(RED_TREASURE_CHEST,1);
			...
			else if (pRankInfo->m_bZone == ZONE_JURAID_MOUNTAIN)
			{
				if (m_sEventRoom == pUser->GetEventRoom())
				{
					if (g_pMain->pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] == pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] && g_pMain->pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] == pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()])
					...
					if ((pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] > pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] && pUser->GetNation() == ELMORAD)
						|| (pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] > pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] && pUser->GetNation() == KARUS))
						nChangeExp = int64((pUser->GetLevel()-20) * (3000 + 100/* Temp Score */* 1000));
				}
			}
#if 0
...
#endif
			if (nChangeExp > 0 && m_sEventRoom == pUser->GetEventRoom())
			{
				if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR)
...
void CGameServerDlg::TempleQuestEventTeleportUsers(CUser *pUser)
{
	if (pUser == nullptr || !pUser->isInGame())
		return;
	...
	case TEMPLE_EVENT_MONSTER_STONE:
			if (!pQuestEvent.isAttackable)
				pQuestEvent.isAttackable = true;
			pQuestEvent.m_eEventTime = QUEST_EVENT_MONSTER_STONE_TIME;
		break;
	}

	if (pUser->GetLevel() >= 1 && pUser->GetLevel() <= 19)
	{
		ZoneID = ZONE_STONE1;
		pUser->m_MonsterStoneFamily = 1;
	}
	else if (pUser->GetLevel() >= 20 && pUser->GetLevel() <= 29)
	{
		ZoneID = ZONE_STONE1;
		pUser->m_MonsterStoneFamily = 2;
	}
	else if (pUser->GetLevel() >= 30 && pUser->GetLevel() <= 39)
	{
		ZoneID = ZONE_STONE1;
		pUser->m_MonsterStoneFamily = 3;
	}
	else if (pUser->GetLevel() >= 40 && pUser->GetLevel() <= 49)
	{
		ZoneID = ZONE_STONE1;
		pUser->m_MonsterStoneFamily = 4;
	}
	else if (pUser->GetLevel() >= 50 && pUser->GetLevel() <= 59)
	{
		ZoneID = (uint16) myrand(ZONE_STONE1,ZONE_STONE2);

		if (ZoneID == ZONE_STONE1)
			pUser->m_MonsterStoneFamily = 4;
		else
			pUser->m_MonsterStoneFamily = 5;
	}
	else if(pUser->GetLevel() >= 60 && pUser->GetLevel() <= 65)
	{
		ZoneID = ZONE_STONE2;
		pUser->m_MonsterStoneFamily = (uint8) myrand(6,8);
	}
	else if(pUser->GetLevel() >= 66 && pUser->GetLevel() <= 69)
	{
		ZoneID = ZONE_STONE2;
		pUser->m_MonsterStoneFamily = (uint8) myrand(8,9);
	}
	else if(pUser->GetLevel() >= 70 && pUser->GetLevel() <= 73)
	{
		pUser->m_MonsterStoneFamily = (uint8) myrand(9,10);

		if (pUser->m_MonsterStoneFamily == 9)
			ZoneID = ZONE_STONE2;
		else
			ZoneID = ZONE_STONE3;
	}
	else if(pUser->GetLevel() >= 74 && pUser->GetLevel() <= 77)
	{
		ZoneID = ZONE_STONE3;
		pUser->m_MonsterStoneFamily = (uint8) myrand(10,12);
	}
	else if(pUser->GetLevel() >= 78 && pUser->GetLevel() <= m_byMaxLevel)
	{
		ZoneID = ZONE_STONE3;
		pUser->m_MonsterStoneFamily = 13;
	}
...
void CGameServerDlg::TempleEventSendActiveEventTime(CUser *pUser)
{
	...
	if(m_nTempleEventRemainSeconds > 0 && !pUser->isEventUser())
	{
		if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR)
		{
			std::string EventNotice;
			EventNotice = string_format("The National Defence Combat Border Defence War Start.");
			result.Initialize(WIZ_CHAT);
			result << uint8(PUBLIC_CHAT) << pUser->GetNation() << pUser->GetSocketID() << uint8(0) << EventNotice;
		}
		else if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN)
		{
			std::string EventNotice;
			EventNotice = string_format("Gate Keeper of Islant mountain: 70 above level Warriors!! I Will let you enter the cave now.");
			result.Initialize(WIZ_CHAT);
			result << uint8(PUBLIC_CHAT) << pUser->GetNation() << pUser->GetSocketID() << uint8(0) << EventNotice;
		}
		pUser->Send(&result);
	...
	pUser->TempleOperations(TEMPLE_EVENT_COUNTER2);
}
...
void CGameServerDlg::TempleEventReset()
{
	...
	if(m_TempleEventUserArray.GetSize() > 0)
...
void CGameServerDlg::Announcement(uint16 type, int nation, int chat_type, CUser* pExceptUser, CNpc *pExpectNpc)
{
	...
	case IDS_PRE_BATTLE_ANNOUNCEMENT:
		...
		break;
	case IDS_SIEGE_WAR_TIME_ANNOUNCEMENT:
		ZoneID = ZONE_DELOS;
		GetServerResource(IDS_SIEGE_WAR_TIME_ANNOUNCEMENT, &chatstr, m_SiegeWarNoticeTime);
		break;
	case IDS_SIEGE_WAR_CLAN_ANNOUNCEMENT:
		{
			if (m_KnightsSiegeWarfareArray.GetSize() < 1)
				return;
			...
			CKnights *pKnights = g_pMain->GetClanPtr(pKnightSiegeWar->sMasterKnights);
			ZoneID = ZONE_DELOS;
			GetServerResource(IDS_SIEGE_WAR_CLAN_ANNOUNCEMENT, &chatstr, pKnights == nullptr ? "***" : pKnights->GetName().c_str(), m_SiegeWarNoticeTime);
		}
		break;
	}
...
uint16 CGameServerDlg::GetKnightsAllMembers(uint16 sClanID, Packet & result, uint16 & pktSize, bool bClanLeader)
{
	...
	foreach_array (i, pKnights->m_arKnightsUser)
	{
		...
		CUser* pCurrent = g_pMain->GetUserPtr(p->strUserName, TYPE_CHARACTER);

		if( pCurrent == nullptr )
		{
			if (!g_DBAgent.LoadKnightsAllMembers(sClanID,p))
				continue;

			result << p->strUserName << p->m_sFame << p->m_bLevel << p->m_sClass << uint8(g_pMain->GetUserPtr(p->strUserName, TYPE_CHARACTER) == nullptr ? 0 : 1);
		}
		else
		{
			result << pCurrent->GetName() << pCurrent->GetFame() << pCurrent->GetLevel() << pCurrent->m_sClass << uint8(g_pMain->GetUserPtr(p->strUserName, TYPE_CHARACTER) == nullptr ? 0 : 1);
		}
		count++;
	}
...
void CGameServerDlg::WritePUSLogFile(std::string & logMessage)
{
	AddLogRequest(logMessage,LogTypes::PUSLOG);
}
...
void CGameServerDlg::ReloadKnightAndUserRanks()
{
	...
	LoadKnightsSiegeWarsTable(true);
...
void CGameServerDlg::TerminationFinish(uint16 EventRoom)
{
	uint8 m_bTerminationFinish = 20;
	...
	if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR) 
	{
		foreach_stlmap(itr, m_TempleEventUserArray) 
		{
			...
			if (EventRoom > 0 && EventRoom != pUser->GetEventRoom())
				continue;

			if (g_pMain->pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] == pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] && g_pMain->pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] == pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()])
				result << uint8(TEMPLE_EVENT_FINISH) << uint8(ELMORAD) << uint8(0) << uint8(Nation::ALL) << m_bTerminationFinish << uint32(0);

			if (pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] > pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] && pUser->GetNation() == ELMORAD) 
				result << uint8(TEMPLE_EVENT_FINISH) << uint8(KARUS) << uint8(0) << uint8(ELMORAD) << m_bTerminationFinish << uint32(0);	

			if (pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] > pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] && pUser->GetNation() == KARUS)		
				result << uint8(TEMPLE_EVENT_FINISH) << uint8(ELMORAD) << uint8(0) << uint8(KARUS) << m_bTerminationFinish << uint32(0);	

			g_pMain->Send_Zone(&result, ZONE_BORDER_DEFENSE_WAR, pUser, Nation::ALL, pUser->GetEventRoom());
		}
	}
	else if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_CHAOS)
	{
		foreach_stlmap(itr, m_TempleEventUserArray) 
		{
			...
			g_pMain->Send_Zone(&result, ZONE_CHAOS_DUNGEON, pUser, Nation::ALL, pUser->GetEventRoom());
		}
	}
	else if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN) 
	{
		foreach_stlmap(itr, m_TempleEventUserArray) 
		{
			...
			g_pMain->Send_Zone(&result, ZONE_JURAID_MOUNTAIN,  pUser, Nation::ALL, pUser->GetEventRoom());
		}
	}
}