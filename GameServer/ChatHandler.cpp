...
void CUser::InitChatCommands()
{
	static Command<CUser> commandTable[] = 
	{
		...
		{ "prison",             &CUser::HandlePrisonCommand,                    "User imprisoned" },
		...
		{ "nt",					&CUser::HandleNtsCommand,						"Nation Change" },
...
void CUser::SendDeathNotice(Unit * pKiller, DeathNoticeType noticeType) 
{
	...
	if(noticeType == DeathNoticeCoordinates || noticeType == DeathNoticeRival)
...
COMMAND_HANDLER(CUser::HandleNtsCommand)
{
	if (!isGM())
		return false;
...
COMMAND_HANDLER(CUser::HandlePrisonCommand)
{
	...
	if (vargs.empty())
	{
		g_pMain->SendHelpDescription(this, "+prison Username");
		return true;
	}
	...
	if (pUser == nullptr)
	{
		g_pMain->SendHelpDescription(this, "Error: User Not in Game.");
		return true;
	}

	_START_POSITION * pStartPosition = g_pMain->GetStartPosition(ZONE_PRISON);
	if (pStartPosition == nullptr)
	{
		g_pMain->SendHelpDescription(this, "Error: Jail Map Not Attached.");
		return false;
	}

	std::string sNoticeMessage = string_format("%s Imprisoned by GM.", strUserID.c_str());
	if (!sNoticeMessage.empty())
		g_pMain->SendNotice(sNoticeMessage.c_str(), Nation::ALL);


	pUser->ZoneChange(ZONE_PRISON, (float)(GetNation() == KARUS ? pStartPosition->sKarusX : pStartPosition->sElmoradX + myrand(0, pStartPosition->bRangeX)),
...
COMMAND_HANDLER(CUser::HandleMonsterSummonCommand)
{
	...
	g_pMain->SpawnEventNpc(sSid, true, GetZoneID(), GetX(), GetY(), GetZ(), 0, sCount);
...
{
	DateTime time;
	std::string sOperatorCommandType = "Unban";
...
COMMAND_HANDLER(CGameServerDlg::HandleReloadTablesCommand)
{
	...
	m_MiningItemArray.DeleteAllData();
	LoadMiningItemTable();

	m_FishingItemArray.DeleteAllData();
	LoadFishingItemTable();

	m_UserSealItemArray.DeleteAllData();
	LoadUserSealTable();
