...
DEFINE_LUA_FUNCTION_TABLE(g_globalFunctions,
						  ...
						  MAKE_LUA_FUNCTION(GetQuestStatus)
						  ...
						  MAKE_LUA_FUNCTION(TempleOperations)
						  MAKE_LUA_FUNCTION(ShowBulletinBoard)
						  ...
						  MAKE_LUA_FUNCTION(GetPremiumTime)
						  ...
						  MAKE_LUA_FUNCTION(EventSoccerMember)
						  MAKE_LUA_FUNCTION(EventSoccerStard)
						  MAKE_LUA_FUNCTION(SpawnEventSystem)
						  );
...
bool CLuaEngine::ExecuteScript(CUser * pUser, CNpc * pNpc, int32 nEventID, int8 bSelectedReward, const char * filename)
{
	...
	if (itr == m_scriptMap.end())
	{
		...
		sNoticeMessage = string_format("NpcID : %d - Event ID : %d - Lua : %s", pNpc->m_sSid,nEventID,szPath.c_str());
