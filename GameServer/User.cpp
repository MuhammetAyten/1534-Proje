...
void CUser::Update()
{
	...
	if (m_bMonsterStoneTime + PLAYER_GENIE_INTERVAL < UNIXTIME && isMonsterSummonActive() && isMonsterStoneActive() && isInTempleQuestEventZone())
	{
		...
		case TEMPLE_EVENT_MONSTER_STONE:
			{
				foreach_stlmap_nolock (itr, g_pMain->m_MonsterStoneListInformationArray)
				{
					if (itr->second->ZoneID == GetZoneID() 
						&& itr->second->Family == m_MonsterStoneFamily)
						g_pMain->SpawnEventNpc(itr->second->sSid, itr->second->bType == 0 ? true : false, itr->second->ZoneID, itr->second->X, itr->second->Y, itr->second->Z, itr->second->byDirection, itr->second->sCount, 0, PLAYER_MONSTER_STONE_INTERVAL, 0, GetSocketID(), GetEventRoom());
				}
...
void CUser::SendLoyaltyChange(int32 nChangeAmount /*= 0*/, bool bIsKillReward /* false */, bool bIsBonusReward /* false */, bool bIsAddLoyaltyMonthly /* true */)
{
	...
	if (nChangeAmount < 0)
	{
		...
		if (bIsKillReward)
		{
			if (GetZoneID() == ZONE_ARDREAM || GetZoneID() == ZONE_RONARK_LAND_BASE || GetZoneID() == ZONE_BORDER_DEFENSE_WAR || GetZoneID() == ZONE_JURAID_MOUNTAIN)
	...
	else
	{
		...
		if (m_FlashWarBonus > 0 && !isDead())
		{
			nChangeAmount = nChangeAmount * (100 + (10 / m_FlashWarBonus)) / 100;
			nChangeAmountLoyaltyMonthly = nChangeAmountLoyaltyMonthly * (100 + (10 / m_FlashWarBonus)) / 100;
		}
		...
		if (bIsKillReward)
		{
			...
			nChangeAmountLoyaltyMonthly += m_bItemNPBonus + m_bSkillNPBonus;
		...
		if (isInClan())
		{
			...
			if (pKnights != nullptr && pKnights->m_byFlag >= ClanTypeAccredited5 && pKnights->GetClanPointMethod() == 0 && !bIsBonusReward)
			{
				if (pKnights->m_sMembers > 0 && pKnights->m_sMembers <= MAX_CLAN_USERS)
				{
					...
					if (isAlive())
						CKnightsManager::AddUserDonatedNP(GetClanID(), GetName(), nClanLoyaltyAmount, true);
				}
			}
		}
	}
...
void CUser::SendMyInfo()
{
	...
	else 
	{
		...
		if(pKnights->GetAllianceID() > 0 ) // Eğer Allyclanda ise 
		{
			CKnights *pKnightsAlly = g_pMain->GetClanPtr(pKnights->GetAllianceID());
			if(pKnightsAlly != nullptr)
			{
				result << uint16(pKnightsAlly->m_sCape)  // Üst clan pelerinini kullan
					<< pKnightsAlly->m_bCapeR << pKnightsAlly->m_bCapeG << pKnightsAlly->m_bCapeB << uint8(0);

			}
			else 
			{
				result << uint16(pKnights->m_sCape)
...
void CUser::SendUserSealItem(Packet & pkt, uint32 nUserSeal)
{
	_USER_SEAL * pUserSeal = g_pMain->m_UserSealItemArray.GetData(nUserSeal);

	if (pUserSeal != nullptr)
	{
		pkt.DByte();

		pkt << pUserSeal->SealSerial
			<< pUserSeal->sCharID
			<< uint8(pUserSeal->sClass)
			<< uint8(pUserSeal->bLevel)
			<< uint16(pUserSeal->sExpPercent)
			<< uint16(pUserSeal->bRaceSatis);
	}
	else
		pkt << uint32(0);
}
...
void CUser::ServerNews()
{
	...
	ServerNews2 = string_format("Moradon Resurrection Server Time : %02d-%02d-%04d  %02d:%02d", time.GetDay(), time.GetMonth(), time.GetYear(), time.GetHour(), time.GetMinute());
	ServerNews1 = string_format("Serverimiz 'ALPHAGUARD' tarafından korunmaktadır !");
	Welcome = string_format("Hello %s, Welcome to the 1534 Moradon Resurrection Beta", GetName().c_str());
...
void CUser::SetZoneAbilityChange(uint16 sNewZone)
{
	...
	case ZONE_MORADON5:
		if (pSiegeWarFare != nullptr)
			pMap->SetTariff((uint8)pSiegeWarFare->sMoradonTariff);
		break;
	case ZONE_DELOS:
		if (pSiegeWarFare != nullptr)
			pMap->SetTariff((uint8)pSiegeWarFare->sDellosTariff);
		break;
	default:
		break;
	}
	...
	if (GetZoneID() == ZONE_DELOS)
		g_pMain->CastleSiegeWarFlag(this);
	...
	if (sNewZone == ZONE_BIFROST || sNewZone == ZONE_BATTLE4 || sNewZone == ZONE_BATTLE5 || sNewZone ==  ZONE_RONARK_LAND)
...
{
	/*
	Fazla user girdiğinde dc problemi olacak
	ona göre userin region içinde gördüğü userlerin
	packet verilerine göre kontroller yazılması gerekecek
	*/
...
void CUser::RequestNpcIn(Packet & pkt)
{
	...
	for (int i = 0; i < npc_count; i++)
	{
		...
		CNpc *pNpc = g_pMain->GetNpcPtr(NpcID);
		if (pNpc == nullptr || pNpc->isDead())
			continue;

		if (GetEventRoom() > 0 && pNpc->GetEventRoom() != GetEventRoom() 
...
void CUser::RecvUserExp(Packet & pkt)
{
		...
		|| (pParty = g_pMain->GetPartyPtr(GetPartyID())) == nullptr)
	{
		...
		if (iNpcExp > 0)
		{
			...
			if (Jackpot > 90)
			{
				if(g_pMain->m_sJackExpPots)
					JackPotExp(nFinalExp);
				else
					ExpChange(nFinalExp,false);
			}
			else
				ExpChange(nFinalExp,false);
		}
	...
	foreach (itr, partyUsers)
	{
		...
		if (iNpcExp > 0)
		{
			...
			{
				if(g_pMain->m_sJackExpPots)
					pUser->JackPotExp(iExp);
				else
					pUser->ExpChange(iExp,false);
			}
			else
				pUser->ExpChange(iExp,false);
		}
...
{	
	...
	if (iExp > 0)
	{
		if (!bIsBonusReward)
		{
			...
			iExp = iExp * (m_sExpGainAmount) /100;

			if (m_FlashExpBonus > 0 && !isDead())
				iExp = iExp * (10 / m_FlashExpBonus) /100;
	...
	result << uint8(1) << uint32(m_iExp); // NOTE: Use proper flag
	Send(&result);

	// If we've lost XP, save it for possib/downle refund later.
	if (iExp < 0)
		m_iLostExp = -iExp;
}
...
void CUser::LevelChange(uint8 level, bool bLevelUp)
{
	...
	result	<< GetSocketID()
		...
		<< m_iMaxHp << m_sHp 
		<< m_iMaxMp << m_sMp
		<< uint16(m_sMaxWeight) << uint16(m_sItemWeight);
...
void CUser::PointChange(Packet & pkt)
{
	...
	SetUserAbility();
	result << m_iMaxHp << m_iMaxMp << m_sTotalHit << uint16(m_sMaxWeight);
	Send(&result);
	SendItemMove(1);
}
...
void CUser::SetUserAbility(bool bSendPacket /*= true*/)
{
	...
	if (isWarrior())
	{
		...
		if (CheckSkillPoint(PRO_SKILL2, 5, 14))
			...
			bDefenseBonus = 50;
		else if (CheckSkillPoint(PRO_SKILL2, 70, g_pMain->m_byMaxLevel))
		{
		...
		else if (CheckSkillPoint(PRO_SKILL2, 40, g_pMain->m_byMaxLevel))
			bResistanceBonus = 90;
...
void CUser::BundleOpenReq(Packet & pkt)
{
	...
	if (pBundle == nullptr
		|| !isInRange(pBundle->x, pBundle->z, MAX_LOOT_RANGE)){
			result << bundle_index << uint8(0); // This box is empty.
			Send(&result);
			return;
		}
...
void CUser::SendNotice()
{
	...
	if(m_FlashExpBonus > 0)
		AppendNoticeEntry(result,count,s_FlashNotice,"Exp Flash");
	else if( m_FlashDcBonus > 0)
		AppendNoticeEntry(result,count,s_FlashNotice,"Dc Flash");
	else if( m_FlashWarBonus > 0)
		AppendNoticeEntry(result,count,s_FlashNotice,"War Flash");
...
void CUser::OperatorCommand(Packet & pkt)
{
	...
	case OPERATOR_BAN_ACCOUNT: // ban account is meant to call a proc to do so
		...
		sNoticeMessage = string_format("%s is currently blocked for illegal software.", strUserID.c_str());
		g_pMain->WriteBanListLogFile(string_format("%d:%d:%d || Sender=%s,Command=%s,User=%s,Period=Permanent,Zone=%d(%d,%d)\n",
			time.GetHour(),time.GetMinute(),time.GetSecond(),GetName().c_str(),sOperatorCommandType.c_str(),strUserID.c_str(),GetZoneID(),uint16(GetX()),uint16(GetZ())));
		break;
	case OPERATOR_MUTE:
		...
		sNoticeMessage = string_format("[%s] The chatting will be disabled.", strUserID.c_str());
		g_pMain->WriteBanListLogFile(string_format("%d:%d:%d || Sender=%s,Command=%s,User=%s,Period=Permanent,Zone=%d(%d,%d)\n",
			time.GetHour(),time.GetMinute(),time.GetSecond(),GetName().c_str(),sOperatorCommandType.c_str(),strUserID.c_str(),GetZoneID(),uint16(GetX()),uint16(GetZ())));
		break;
	...
	case OPERATOR_UNMUTE:
		...
		sNoticeMessage = string_format("[%s] The chatting has been enabled.", strUserID.c_str());
		g_pMain->WriteBanListLogFile(string_format("%d:%d:%d || Sender=%s,Command=%s,User=%s,Period=Permanent,Zone=%d(%d,%d)\n",
			time.GetHour(),time.GetMinute(),time.GetSecond(),GetName().c_str(),sOperatorCommandType.c_str(),strUserID.c_str(),GetZoneID(),uint16(GetX()),uint16(GetZ())));
		break;
	}
...
{
	if (pEvent->sBelong != 0 && pEvent->sBelong != GetNation() || pEvent->sBelong != 3 && pEvent->sIndex != 301 && pEvent->sIndex != 302)
		return false;
...
bool CUser::LogLeverBuringLog(_OBJECT_EVENT *pEvent, int nid) 
{
	...
	Guard lock(g_pMain->m_arNpcArray.m_lock);
	foreach_stlmap(itr, g_pMain->m_arNpcArray)
	{
		pGateNpc = itr->second;
		if (pGateNpc == nullptr 
...
void CUser::ResetGMVisibility()
{
	...
	if (m_bAbnormalType != ABNORMAL_INVISIBLE)
		...
		StateChangeServerDirect(5, ABNORMAL_INVISIBLE);
}
...
void CUser::GoldGain(uint32 gold, bool bSendPacket /*= true*/, bool bApplyBonus /*= false*/)
{
	...
	if (bApplyBonus)
	{
		...
		if (m_FlashDcBonus > 0 && !isDead())
			gold = gold * (m_bNoahGainAmount + (10 / m_FlashDcBonus)) / 100;
	}
...
void CUser::OnDeath(Unit *pKiller)
{
	...
	if (pKiller != nullptr)
	{
		...
		if (pKiller->isNPC())
		{
			...
			if ((isQuestEventUser() && (GetZoneID() == ZONE_STONE1 || GetZoneID() == ZONE_STONE2 || GetZoneID() == ZONE_STONE3)) 
				|| (isEventUser() && (GetZoneID() == ZONE_JURAID_MOUNTAIN))
				|| (pNpc->GetZoneID() == ZONE_KROWAZ_DOMINION && (pNpc->GetType() == NPC_ROLLINGSTONE || pNpc->GetProtoID() == SAW_BLADE_SSID)))
			{
				if (pNpc->GetZoneID() == ZONE_KROWAZ_DOMINION && (pNpc->GetType() == NPC_ROLLINGSTONE || pNpc->GetProtoID() == SAW_BLADE_SSID))
		...
		else
		{
			...
			{
				if (pUser->isEventUser() && (pUser->isInTempleEventZone(pUser->GetZoneID())))
				{
					noticeType = DeathNoticeCoordinates;
					...
					else if (pUser->GetZoneID() == ZONE_BORDER_DEFENSE_WAR 
						|| pUser->GetZoneID() == ZONE_JURAID_MOUNTAIN)
					{
						Packet result(WIZ_EVENT, uint8(TEMPLE_EVENT_BORDER_COUNTER));

						pUser->GetNation() == ELMORAD ? g_pMain->pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()]++ : g_pMain->pTempleEvent.KarusDeathCount[pUser->GetEventRoom()]++;
						pUser->m_iLoyaltyDaily++;
						pUser->UpdatePlayerRank();
						
						// Loyalty should be awarded on kill.
						if (!pUser->isInParty())
							pUser->LoyaltyChange(GetID(), 0);
						...
						if (GetNation() == KARUS && !pUser->isInParty() && pUser->isPlayer())
							pUser->QuestV2MonsterCountAdd(KARUS);
						else if(GetNation() == ELMORAD && !pUser->isInParty() && pUser->isPlayer())
						...
						if (pUser->isInParty() && (GetNation() == KARUS || GetNation() == ELMORAD))
						{
							...
							for (int i = 0; i < MAX_PARTY_USERS; i++)
							{
								...
								if (PartyUser->GetNation() == KARUS && PartyUser->isPlayer())
									PartyUser->QuestV2MonsterCountAdd(KARUS);
								else if (PartyUser->GetNation() == ELMORAD && PartyUser->isPlayer())
						...
						g_pMain->Send_Zone(&result, pUser->GetZoneID(), pUser, Nation::ALL, pUser->GetEventRoom());

						if (g_pMain->pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] > 59 || g_pMain->pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] > 59)
							g_pMain->TempleEventFinish(pUser->GetEventRoom());
					}
				}
				else if (pUser->GetZoneID() == ZONE_DELOS && g_pMain->m_byBattleOpen == SIEGE_BATTLE)
				{
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							if (GetMap()->isWarZone() && g_pMain->isWarOpen())
								noticeType = DeathNotice;

							if (g_pMain->isWarOpen() && (pUser->GetZoneID() >= ZONE_KARUS && pUser->GetZoneID() <= ZONE_KARUS4 
								|| pUser->GetZoneID() >= ZONE_ELMORAD && pUser->GetZoneID() <= ZONE_ELMORAD4))
								noticeType = DeathNotice;
...
void CUser::HandleHelmet(Packet & pkt)
{
	...
	result << m_bIsHidingHelmet << m_bIsHidingCospre << GetSocketID() << uint16(0);
...
void CUser::HandleMiningAttempt(Packet & pkt)
{
  if (!isMining())
	...
	if (resultCode == MiningResultSuccess)
	{
		...
		if (rate >= 1 && rate  <= 10 && random <= 1000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);

			_MINING_ITEM * pMining = g_pMain->m_MiningItemArray.GetData(rate);
			...
			else if (pMining->nRate == rate)
			{
				GiveItem(pMining->nItemID,pMining->nCount);
				sEffect = 13081; // "Item" effect
			}
		}
		else if (rate >= 10 && rate <= 20 && random <= 2000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
		...
		else if (rate >= 20 && rate <= 30 && random <= 3000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
		...
		else if (rate >= 30 && rate <= 40 && random <= 4000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
		...
		else if (rate >= 40 && rate <= 50 && random <= 5000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
		...
		else if (rate >= 50 && rate <= 60 && random <= 6000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
		...
		else if (rate >= 60 && rate <= 70 && random <= 7000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
		...
		else if (rate >= 70 && rate <= 80 && random <= 8000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
		...
		else if (rate >= 80 && rate <= 90 && random <= 9000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
		...
		else if (rate >= 90 && rate <= 100 && random <= 10000)
		{
			if (pTable->m_iNum == GOLDEN_MATTOCK)
				rate = (rate + 100);
...
void CUser::HandleFishingAttempt(Packet & pkt)
{
	...
	if (resultCode == MiningResultSuccess)
	{
		...
		if (rate >= 1 && rate  <= 10 && random <= 1000)
		{
			...
			_FISHING_ITEM * pFishing = g_pMain->m_FishingItemArray.GetData(rate);
			...
			else if (pFishing->nRate == rate)
			{
				GiveItem(pFishing->nItemID,pFishing->nCount);
...
{
	if (bPremiumType <= 0 || sPremiumTime <= 0)
		return;
	...
	else if (GetPremium() > 0 && GetPremium() == bPremiumType)
	{
		m_bPremiumType = bPremiumType;
		m_sPremiumTime += sPremiumTime * 24;
	}
				...
				result << opcode << type;
					...
					case 4:
						if (GetZoneID() >= ZONE_MORADON && GetZoneID() <= ZONE_MORADON5)
						{
							...
							g_pMain->UpdateSiegeTax(ZoneID , tarrif);
							result << uint16(1) << tarrif << uint8(ZoneID);
...
void CUser::ExpFlash()
{
	...
	if (GetPremium() == 11 && m_FlashExpBonus < 100)
	{
		std::string val;
		m_FlashExpBonus += 10;
		...
		memset(&s_FlashNotice, 0 ,sizeof(s_FlashNotice));
		strcpy(s_FlashNotice,val.c_str());
		SendNotice();
	}
}
...
void CUser::DcFlash()
{
	if (GetPremium() == 10 && m_FlashDcBonus < 100)
	{
		std::string val;	
		m_FlashDcBonus += 10;
...
void CUser::WarFlash()
{
	if (GetPremium() == 12 && m_FlashWarBonus < 100)
	{
		std::string val;
		m_FlashWarBonus += 10;
...
//Alıntı DüslerinSokağı Can Abiye Teşekkürler..
...
void CUser::ShowBulletinBoard()
{
	...
	uint8 count = 0;
	result << uint8(count) << uint16(1000);	
	result.DByte();
	CKingSystem *pKingSystem = g_pMain->m_KingSystemArray.GetData(GetNation());

	if (pKingSystem)
	{
		result << pKingSystem->m_strKingName
			<< uint16(0);

		CKnights *pKnights = g_pMain->GetClanPtr(g_DBAgent.GetClanIDWithCharID(pKingSystem->m_strKingName));

		if (pKnights != nullptr)
		{
			result << uint16(pKnights->m_sIndex)
				<< uint16(pKnights->m_sMarkVersion)
				<< pKnights->m_strName
				<< uint16(0);
		}
	...
	g_DBAgent.LoadLadderRankList(result, GetNation(), count);
	g_DBAgent.LoadKnightsLeaderList(result,GetNation(),count);
	
	for (int i = 2311; i < 0; i++)
	{
		result << uint16(i)// rankid
			<< "isim1"
			<< uint16(0)
			<< uint16(i)   // clanid
			<< int16(-1)   // mark
			<< "Clan_test1"
			<< uint16(i);  // Kill Enemies
		count++;
	}

	for (int i = 2321; i < 0; i++)
	{
		result << uint16(i)// rankid
			<< "isim2"
			<< uint16(0)
...
void CUser::SpawnEventSystem(uint16 sSid, uint8 bIsmonster, uint8 byZone, float fX, float fY, float fZ)
{
	if (bIsmonster >= 0 && bIsmonster <= 1)
		g_pMain->SpawnEventNpc(sSid, bIsmonster == 0 ? true : false, byZone, fX, fY, fZ, 0, 1, 0, 1 * HOUR);
	else
		return;
}