	...
	case ELECTION_TYPE_TERM_ENDED:
		{
				...
				&& bCurMinute == dt.GetMinute())
			{
				...
				m_sYear = m_sNextYear;
				m_byMonth = m_byNextMonth;
				m_byDay = m_byNextDay;
				m_byHour = m_byNextHour;
				m_byMinute = m_byNextMinute;
...
{
	CUser *pUser = g_pMain->GetUserPtr(m_strKingName, TYPE_CHARACTER);
	...
	pUser = nullptr;
	pUser = g_pMain->GetUserPtr(m_strKingName, TYPE_CHARACTER);
	...
	int perc = int((float(m_KingVotes) / float(m_TotalVotes)) * 100);
	if(m_TotalVotes == 0)
		perc = 100;
	else
		perc = int((float(m_KingVotes) / float(m_TotalVotes)) * 100);

	// Announce the new elected king.
	g_pMain->SendFormattedResource(IDS_KING_ELECTION_RESULT_MESSAGE, m_byNation, true, perc, m_strKingName.c_str());
	return;
}
#pragma endregion
...
void CKingSystem::DetermineKingCandidaciesList()
{
	...
	{
		// if the senator is nominated by some other senator already
		// then we do not need to add him the the king cancidate list.
		if(m_candidateList.find(itr->first) != m_candidateList.end()
			|| m_nominationList.find(itr->first) != m_nominationList.end())
			continue;

		// Copy the information we need from our senator list entry
		_KING_ELECTION_LIST * pEntry = new _KING_ELECTION_LIST;
		memcpy(pEntry, itr->second, sizeof(_KING_ELECTION_LIST));
		m_candidateList.insert(make_pair(itr->first, pEntry));

		_KING_NOMINATION_LIST * pEntryNomination = new _KING_NOMINATION_LIST;
		pEntryNomination->strNominator = itr->first;
		pEntryNomination->strNominee = itr->first;
		m_nominationList.insert(make_pair(itr->first, pEntryNomination));
	} 
...
void CKingSystem::InsertNominee(std::string & strNominator, std::string & strNominee, uint16 sNomineeKnights)
{
	...
	g_pMain->SendFormattedResource(IDS_KING_CANDIDACY_RECOMMEND_MESSAGE, m_byNation, true, strNominator.c_str(), strNominee.c_str());
	...
	case 2:
		{
			...
			if(!pUser->isInClan() && pUser->m_iLoyalty < MIN_NP_VOTER)
			{
				result << int16(-4);
				pUser->Send(&result);
				return;
			...
			if(pUser->isInClan())
			{
				if(pUser->m_pKnightsUser)
				{
					if((pUser->m_pKnightsUser->nDonatedNP + pUser->m_iLoyalty) < MIN_NP_VOTER)
					{
						result << int16(-4);
						pUser->Send(&result);
						return;
...
void CKingSystem::CandidacyResign(CUser * pUser, Packet & pkt) 
{
	...
	m_resignedCandidateList.insert(nominationItr->second->strNominator);
